...
public class Thinlet extends Container implements Runnable, Serializable {
    ...
    private transient Color c_tooltip = Color.yellow;
    ...
    private static Method renderinghint;
    ...
    static {
        try {
            ...
            TXT_AA = new Object[] { hintsclass.getField("KEY_TEXT_ANTIALIASING").get(null), hintsclass.getField("VALUE_TEXT_ANTIALIAS_ON").get(null) };
            G_AA = new Object[] { hintsclass.getField("KEY_ANTIALIASING").get(null), hintsclass.getField("VALUE_ANTIALIAS_ON").get(null) };
        } catch (Exception exc) {
            exc.printStackTrace();
            /* not 1.4 */}
    }
    {
        ...
        setColors(0xe6e6e6, 0x000000, 0xffffff, 0x909090, 0xb0b0b0, 0xededed, 0xb9b9b9, 0x89899a, 0xc5c5dd, DEFAULT_RESIZE_BORDER_COLOR);

        // disable global focus-manager for this component in 1.4
        if (MOUSE_WHEEL != 0) {
            try {
                getClass().getMethod("setFocusTraversalKeysEnabled", new Class[] { Boolean.TYPE }).invoke(this, new Object[] { Boolean.FALSE });
            } catch (Exception exc) { /* never */
            }
        }
        // set listeners flags
        enableEvents(AWTEvent.COMPONENT_EVENT_MASK | AWTEvent.FOCUS_EVENT_MASK | AWTEvent.KEY_EVENT_MASK | AWTEvent.MOUSE_EVENT_MASK
     ...
     *
     * @param background
     *            the backround of panels (dialogs, desktops), and disabled controls, not editable texts, lines between list items (the default value if
     *            <i>#e6e6e6</i>)
     * @param text
     *            for text, arrow foreground (<i>black</i> by default)
     * @param textbackground
     *            the background of text components, and lists (<i>white</i> by default)
     * @param border
     *            for outer in inner borders of enabled components (<i>#909090</i> by default)
     * @param disable
     *            for text, border, arrow color in disabled components (<i>#b0b0b0</i> by default)
     * @param hover
     *            indicates that the mouse is inside a button area (<i>#ededed</i> by default)
     * @param press
     *            for pressed buttons, gradient image is calculated using the background and this press color (<i>#b9b9b9</i> by default)
     * @param focus
     *            for text caret and rectagle color marking the focus owner (<i>#89899a</i> by default)
     * @param select
     *            used as the background of selected text, and list items, and in slider (<i>#c5c5dd</i> by default)
     * @param resizeBorder
     *            used as color for the border decorator for resizable dialogs. (<i>DEFAULT_RESIZE_BORDER_COLOR</i> by default)
     */
    ...
    /**
     * Sets the only one font used everywhere, and revalidates the whole UI. Scrollbar width/height, spinbox, and combobox button width, and slider size is the
     * same as the font height
     *
     * @param font
     *            the default font is <i>SansSerif</i>, <i>plain</i>, and <i>12pt</i>
     */
    @Override
    ...
    protected void doLayout(Object component) {
        ...
        if ("combobox".equals(classname)) {
            ...
            layoutScroll(component, width + 2, (height - fm.getLeading()) + 2, 0, 0, 0, 0, getBoolean(component, "border", true), 0);
            ...
            for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                ...
                if ("dialog".equals(iclass)) {
                    ...
                    if (get(comp, "bounds") == null) {
                        setRectangle(comp, "bounds", Math.max(0, (bounds.width - d.width) / 2), Math.max(0, (bounds.height - d.height) / 2),
            ...
            if ((comp2 != null) && getBoolean(comp2, "visible", true)) {
                setRectangle(comp2, "bounds", horizontal ? (divider + 5) : 0, horizontal ? 0 : (divider + 5),
                        horizontal ? (bounds.width - 5 - divider) : bounds.width, horizontal ? bounds.height : (bounds.height - 5 - divider));
                doLayout(comp2);
            }
    ...
    private Object popupMenu(Object component) {
        ...
        popup(selected, popup, menubar ? (("bottom" != get(component, "placement")) ? 'D' : 'U') : 'R', menubar ? (menux + menubounds.x) : menux,
                menuy + menubounds.y, menubar ? menubounds.width : menuwidth, menubar ? menuheight : menubounds.height, menubar ? 1 : 3);
        return popup;
    }
     ...
     * @param popupmenu
     */
    public void popupPopup(Object popupmenu, int x, int y) {
     ...
     *
     * @param menu
     *            menubar's menu, menu's menu, or component's popupmenu including items
     * @param popup
     *            created popupmenu
     * @param direction
    ...
    private void popup(Object menu, Object popup, char direction, int x, int y, int width, int height, int offset) {
        ...
        for (Object item = get(menu, ":comp"); item != null; item = get(item, ":next")) {
            ...
            Dimension d = ("separator".equals(itemclass)) ? new Dimension(1, 1) : getSize(item, 8, 4);
    ...
    private void closeCombo(Object combobox, Object combolist, Object item) {
        if ((item != null) && getBoolean(item, "enabled", true)) {
            ...
            // invoke(combobox, item, "action"); BZR
    ...
    private void showTip() {
        ...
        if (("tabbedpane".equals(classname)) || ("menubar".equals(classname)) || (classname == ":popup")) {
            if (insidepart != null) {
    ...
    /**
     * Set viewport (:port) bounds excluding borders, view position and content size (:view), horizontal (:horizontal), and vertical (:vertical) scrollbar
     * bounds
     *
     * @param component
     *            scrollable widget
     ...
     * @param top
     *            top inset (e.g. table header, dialog title, half of panel title)
     * @param left
     ...
     *
     *         list: 0, 0, 0, 0, true, 0 | table: header, ... | dialog: header, 3, 3, 3, true, 0 title-border panel: header / 2, 0, 0, 0, true, head
     */
    private boolean layoutScroll(Object component, int contentwidth, int contentheight, int top, int left, int bottom, int right, boolean border, int topgap) {
        Rectangle bounds = getRectangle(component, "bounds");
        int iborder = border ? 1 : 0;
    ...
    private Dimension getPreferredSize(Object component) {
        ...
        if (("panel".equals(classname)) || ("dialog".equals(classname))) {
            ...
            for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                String iclass = getClass(comp);
                if ((!"dialog".equals(iclass)) && (iclass != ":popup") && (iclass != ":combolist")) {
                    Dimension d = getPreferredSize(comp);
                    size.width = Math.max(d.width, size.width);
                    size.height = Math.max(d.height, size.height);
                }
        ...
        if (("list".equals(classname)) || ("table".equals(classname)) || ("tree".equals(classname))) {
    ...
    private Dimension getFieldSize(Object component) {
        ...
        Font currentfont = (Font) get(component, "font");
        FontMetrics fm = getFontMetrics((currentfont != null) ? currentfont : font);
        return new Dimension(((columns > 0) ? (columns * fm.charWidth('e')) : 76) + 4, fm.getAscent() + fm.getDescent() + 4); // fm.stringWidth(text)
    }
        ...
        if (TXT_AA != null) { // set antialiasing
            try {
                if (renderinghint == null) {
                    renderinghint = g.getClass().getMethod("setRenderingHint", new Class[] { TXT_AA[0].getClass().getSuperclass(), Object.class });
                }
                renderinghint.invoke(g, TXT_AA); // text rendering with
                                                 // antialiasing
    ...
    /**
     * @param clipx
     *            the cliping rectangle is relative to the component's parent location similar to the component's bounds rectangle
     * @param clipy
     * @param clipwidth
     * @param clipheight
     * @throws java.lang.IllegalArgumentException
    ...
    private void paint(Graphics g, int clipx, int clipy, int clipwidth, int clipheight, Object component, boolean enabled) {
        ...
        if ("label".equals(classname)) {
            ...
            paintField(g, clipx, clipy, clipwidth, clipheight, component, bounds.width, bounds.height, focus, enabled, ("passwordfield".equals(classname)), 0);
        } else if ("textarea".equals(classname)) {
            paintScroll(component, classname, pressed, inside, focus, enabled, g, clipx, clipy, clipwidth, clipheight);
        } else if ("tabbedpane".equals(classname)) {
            int i = 0;
            ...
            for (Object tab = get(component, ":comp"); tab != null; tab = get(tab, ":next")) {
                ...
                if (selected != i) {
                    ...
                    paint(tab, r.x + bx, r.y + by, r.width - bw, r.height - bh, g, clipx, clipy, clipwidth, clipheight, (!"bottom".equals(placement)),
                            (!"right".equals(placement)), !stacked && (!"top".equals(placement)), (!"left".equals(placement)), 1, 3, 1, 3, false,
                            tabenabled ? (hover ? 'h' : 'g') : 'd', "left", true, false);

                } else {
                    selectedtab = tab;
                    // paint tabbedpane border
                    paint(tab, ("left".equals(placement)) ? r.width - 1 : 0, stacked ? ((r.y + r.height) - 1) : ("top".equals(placement)) ? r.height - 1 : 0,
                            (horizontal || stacked) ? bounds.width : ((bounds.width - r.width) + 1),
                            stacked ? ((bounds.height - r.y - r.height) + 1) : horizontal ? ((bounds.height - r.height) + 1) : bounds.height, g, true, true,
            ...
            if (selectedtab != null) {
                ...
                paint(selectedtab, r.x, r.y, r.width, r.height, g, clipx, clipy, clipwidth, clipheight, (!"bottom".equals(placement)),
                        (!"right".equals(placement)), !stacked && (!"top".equals(placement)), (!"left".equals(placement)), pv, ph, pv, ph, focus,
            ...
            if ("dialog".equals(classname)) {
                ...
                paintRect(g, 0, 3 + titleheight, bounds.width, bounds.height - 3 - titleheight, c_border, c_press, false, true, true, true, true); // lower part
                                                                                                                                                   // excluding
                                                                                                                                                   // titlebar
                // drawing a visible decorator in the lower right corner of the
                // dialog if it is resizable
                ...
                paint(component, 0, titleheight / 2, bounds.width, bounds.height - (titleheight / 2), g, border, border, border, border, enabled ? 'e' : 'd');
            ...
            if ((tooltipowner != null) && (component == content)) {
                ...
                paintRect(g, r.x, r.y, r.width, r.height, c_border, c_tooltip, true, true, true, true, true);
            ...
            paintRect(g, 0, 0, horizontal ? length : bounds.width, horizontal ? bounds.height : length, enabled ? c_border : c_disable, fg, true, true,
                    horizontal, !horizontal, true);

            paintRect(g, horizontal ? length : 0, horizontal ? 0 : length, horizontal ? (bounds.width - length) : bounds.width,
                    horizontal ? bounds.height : (bounds.height - length), enabled ? c_border : c_disable, bg, true, true, true, true, true);
            ...
            if (focus) {
                g.setColor(c_focus);
                g.drawRect(horizontal ? (length + 2) : 2, horizontal ? 2 : (length + 2), (horizontal ? block : bounds.width) - 5,
            ...
            paintRect(g, lastx, 0, bounds.width - lastx, bounds.height, enabled ? c_border : c_disable, enabled ? c_ctrl : c_bg, bottom, false, !bottom, false,
                    true);
            ...
            for (Object menu = get(get(component, "menu"), ":comp"); menu != null; menu = get(menu, ":next")) {
                ...
                if ("separator".equals(itemclass)) {
                    ...
                    paint(menu, r.x, r.y, bounds.width - 2, r.height, g, clipx, clipy, clipwidth, clipheight, false, false, false, false, 2,
                            ("checkboxmenuitem".equals(itemclass)) ? (block + 7) : 4, 2, 4, false, menuenabled ? (armed ? 's' : 't') : 'd', "left", true,
                            false);
                    if ("checkboxmenuitem".equals(itemclass)) {
                    ...
                    if ("menu".equals(itemclass)) {
                        ...
                        if (accelerator != null) { // TODO
                            g.drawString(accelerator, bounds.width - 4 - getFontMetrics(font).stringWidth(accelerator), r.y + 2 + 10);
                        }
                    }
                }
            }
     ...
     *
     */
    private void paintReverse(Graphics g, int clipx, int clipy, int clipwidth, int clipheight, Object component, boolean enabled) {
        if (component != null) {
            Rectangle bounds = getRectangle(component, "bounds");
     ...
     *
     */
    private void paintField(Graphics g, int clipx, int clipy, int clipwidth, int clipheight, Object component, int width, int height, boolean focus,
            boolean enabled, boolean hidden, int left) {
        boolean editable = getBoolean(component, "editable", true);
        paintRect(g, 0, 0, width, height, enabled ? c_border : c_disable, editable ? c_textbg : c_bg, true, true, true, true, true);
     ...
     */
    private void paintScroll(Object component, String classname, boolean pressed, boolean inside, boolean focus, boolean enabled, Graphics g, int clipx,
        ...
        if ((!"panel".equals(classname)) && (!"dialog".equals(classname)) && ((!"textarea".equals(classname)) || getBoolean(component, "border", true))) {
            paintRect(g, port.x - 1, port.y - 1, port.width + (vneed ? 1 : 2), port.height + (hneed ? 1 : 2), enabled ? c_border : c_disable, c_textbg, true,
                    true, !hneed, !vneed, true);
            if ("table".equals(classname)) {
                Object header = get(component, "header");
                if (header != null) {
                    ...
                    for (int i = 0; i < columnwidths.length; i++) {
                        ...
                        paint(column, x - view.x, 0, width, port.y - 1, g, clipx, clipy, clipwidth, clipheight, true, true, false, lastcolumn, 1, 1, 0, 0,
                                false, enabled ? (insidepart == column ? 'h' : 'g') : 'd', "left", false, false);

                        Object sort = get(column, "sort"); // "none", "ascent",
                                                           // "descent"
                        if ((sort != null) && (!"none".equals(sort))) {
                            paintArrow(g, ((x - view.x) + width) - block, 0, block, port.y, ("ascent".equals(sort)) ? 'S' : 'N');
                        }
                        x += width;
                    }
                    g.setClip(clipx, clipy, clipwidth, clipheight);
    ...
    /**
     * @param column
     *            the column for which we like to set its width to its current smartwidth
     */
     ...
     *            a panel
     */
    private void paint(Object component, String classname, boolean focus, boolean enabled, Graphics g, int clipx, int clipy, int clipwidth, int clipheight,
        ...
        if ("textarea".equals(classname)) {
            ...
            for (Object choice = get(get(component, "combobox"), ":comp"); choice != null; choice = get(choice, ":next")) {
                ...
                paint(choice, r.x, r.y, portwidth, r.height, g, clipx, clipy, clipwidth, clipheight, false, false, false, false, 2, 4, 2, 4, false,
                        getBoolean(choice, "enabled", true) ? ((lead == choice) ? 's' : 't') : 'd', "left", false, false);
            }
        } else if (("panel".equals(classname)) || ("dialog".equals(classname))) {
            for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                paint(g, clipx, clipy, clipwidth, clipheight, comp, enabled);
            ...
            for (Object item = get(component, ":comp"), next = null; item != null; item = next) {
                ...
                paintRect(g, (!"tree".equals(classname)) ? 0 : r.x, r.y, (!"tree".equals(classname)) ? viewwidth : r.width, r.height, c_focus,
                ...
                if (!"table".equals(classname)) { // list or tree
                    ...
                    for (Object cell = get(item, ":comp"); cell != null; cell = get(cell, ":next")) {
                        ...
                        // unfortunatly, for instance, we have to do the width
                        // stuff twice, will be done in paint as well
                        Object header = get(component, "header");
                        if ((header != null) && getBoolean(header, "resizable")) {
                            ...
                            if (text != null) {
                                smartWidth += g.getFontMetrics().stringWidth(text);
                            }
                            if (icon != null) {
                                smartWidth += icon.getWidth(this) + (text != null ? 2 : 0);
                            }
                            int[] smartWidths = (int[]) get(component, PROPERTY_SMARTWIDTHS);
                            if (smartWidth > smartWidths[i]) {
                                smartWidths[i] = smartWidth;
                            }
                        }
                        // column width is defined by header calculated in
                        // layout, otherwise is DEFAULT_COLUMN_WIDTH
     ...
     *
     */
    private void paintRect(Graphics g, int x, int y, int width, int height, Color border, Color bg, boolean top, boolean left, boolean bottom, boolean right,
            boolean horizontal) {
        if ((width <= 0) || (height <= 0)) {
            return;
        }
    ...
    private void fill(Graphics g, int x, int y, int width, int height, boolean horizontal) {
        if (horizontal) {
            ...
            for (int i = 0; i < width; i += block) {
                g.drawImage(hgradient, x + i, (height > block) ? ((y + height) - block) : y, x + Math.min(i + block, width) + evm, y + height + evm, 0, 0,
                        Math.min(block, width - i) + evm, Math.min(block, height) + evm, null);
            }
        } else {
            if (width > block) {
                g.setColor(c_bg);
            ...
            for (int i = 0; i < height; i += block) {
                g.drawImage(vgradient, (width > block) ? ((x + width) - block) : x, y + i, x + width + evm, y + Math.min(i + block, height) + evm, 0, 0,
                        Math.min(block, width) + evm, Math.min(block, height - i) + evm, null);
            }
        }
    }
     ...
     *
     */
    private void paintArrow(Graphics g, int x, int y, int width, int height, char dir, boolean enabled, boolean inside, boolean pressed, String part,
     ...
     */
    private void paint(Object component, int x, int y, int width, int height, Graphics g, boolean top, boolean left, boolean bottom, boolean right, char mode) {
        if ((width <= 0) || (height <= 0)) {
            return;
        }
     ...
     */
    private void paint(Object component, int x, int y, int width, int height, Graphics g, int clipx, int clipy, int clipwidth, int clipheight, boolean top,
    ...
    /**
     * This component can be traversed using Tab or Shift-Tab keyboard focus traversal, although 1.4 replaced this method by <i>isFocusable</i>, so 1.4
     * compilers write deprecation warning
     *
    ...
    /**
     * Dispatches mouse, key, focus, and component events occurring on the <i>Thinlet</i> component internally
     */
    @Override
    protected void processEvent(AWTEvent e) {
        ...
        if ((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_MOVED) || (id == MouseEvent.MOUSE_EXITED) || (id == MouseEvent.MOUSE_PRESSED)
            ...
            popuptrigger |= (me.getButton() == 3);

            int ibutton = me.getButton();
            if (id == MouseEvent.MOUSE_ENTERED) {
                if (mousepressed == null) {
                    findComponent(content, x, y);
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_ENTERED, mouseinside, insidepart);
                }
                ...
                if ((previnside == mouseinside) && (prevpart == insidepart)) {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_MOVED, mouseinside, insidepart);
                } else {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_EXITED, previnside, prevpart);
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_ENTERED, mouseinside, insidepart);
                }
                ...
                if (mousepressed == null) {
                    ...
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_EXITED, mouseexit, exitpart);
                }
                ...
                handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_PRESSED, mousepressed, pressedpart);
                ...
                if (wasin && !isin) {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_EXITED, mousepressed, pressedpart);
                } else if (!same && (popupowner != null) && !wasin) {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, DRAG_EXITED, previnside, prevpart);
                }
                if (isin && !wasin) {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_ENTERED, mousepressed, pressedpart);
                } else if (!same && (popupowner != null) && !isin) {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, DRAG_ENTERED, mouseinside, insidepart);
                }
                if (isin == wasin) {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_DRAGGED, mousepressed, pressedpart);
                }
                ...
                handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, ibutton, MouseEvent.MOUSE_RELEASED, mouserelease, releasepart);
            ...
            if (focusinside && ((popupowner != null) || (focusowner != null))) {
                ...
                boolean controldown = (ke.getModifiers() & msk) != 0;
                //

                boolean control = (keychar <= 0x1f) || ((keychar >= 0x7f) && (keychar <= 0x9f)) || (keychar >= 0xffff) || controldown;
                int keycode = control ? ke.getKeyCode() : 0;
                if ((control == (id == KeyEvent.KEY_PRESSED)) && processKeyPress((popupowner != null) ? popupowner : focusowner, ke.isShiftDown(), controldown,
                        ke.getModifiers(), control ? 0 : keychar, keycode)) {
                    ke.consume();
    ...
    private void checkLocation() {
        ...
        handleMouseEvent(mousex, mousex, 1, false, false, false, 0, MouseEvent.MOUSE_ENTERED, mouseinside, insidepart);
    }
    ...
    private boolean processKeyPress(Object component, boolean shiftdown, boolean controldown, int modifiers, int keychar, int keycode) {
        ...
        if ("combobox".equals(classname)) {
            ...
            if (combolist == null) { // the drop down list is not visible
                boolean editable = getBoolean(component, "editable", true);
                if (editable && processField(component, shiftdown, controldown, modifiers, keychar, keycode, false, false, false)) {
                ...
                if ((keycode == KeyEvent.VK_UP) || (keycode == KeyEvent.VK_DOWN) || (keycode == KeyEvent.VK_PAGE_UP) || (keycode == KeyEvent.VK_PAGE_DOWN)
                    ...
                    return (true);
                } else if (!processField(component, shiftdown, controldown, modifiers, keychar, keycode, false, false, false)) {
        ...
        if ("button".equals(classname)) {
            if ((keychar == KeyEvent.VK_SPACE) || ((keycode == KeyEvent.VK_ENTER) && (get(component, "type") == "default"))
            ...
            return processField(component, shiftdown, controldown, modifiers, keychar, keycode, false, ("passwordfield".equals(classname)), false);
        } else if ("textarea".equals(classname)) {
            char[] chars = (char[]) get(component, ":text");
            int start = getInteger(component, "start", 0);
            int end = getInteger(component, "end", 0);
            ...
            } else if ((keycode == KeyEvent.VK_UP) || (keycode == KeyEvent.VK_PAGE_UP) || (keycode == KeyEvent.VK_DOWN) || (keycode == KeyEvent.VK_PAGE_DOWN)) {
                Font currentfont = (Font) get(component, "font");
                FontMetrics fm = getFontMetrics((currentfont != null) ? currentfont : font);
                int fh = fm.getHeight();
                int y = 0;
            ...
            KeyEvent e = new KeyEvent(this, KeyEvent.KEY_PRESSED, System.currentTimeMillis(), 0, keycode, (char) keychar);
    ...
    /**
     * Process keyboard events for textfield, passwordfield, textarea, combobox, and spinbox
     *
     * @param multiline
     ...
     */
    private boolean processField(Object component, boolean shiftdown, boolean controldown, int modifiers, int keychar, int keycode, boolean multiline,
            boolean hidden, boolean filter) {
        String text = getString(component, "text", "");
        int start = getInteger(component, "start", 0);
        int end = getInteger(component, "end", 0);
        boolean editable = getBoolean(component, "editable", true);
     ...
     */
    private boolean changeField(Object component, String text, String insert, int movestart, int moveend, int start, int end) {
    ...
    /**
     * Search for the next/first appropriate item starting with the collected string or the given single character
     *
     * @param keychar
        ...
        if (keychar != 0) {
            ...
            for (; i < 2; i++) { // 0: find the long text, 1: the stating
                ...
                for (int j = 0; j < 2; j++) { // 0: lead to last, 1: first to
                                              // lead
                    for (Object item = (j == 0) ? ((i == 0) ? lead : getNextItem(component, lead, recursive)) : get(component, ":comp"); (j == 0)
                            ? (item != null)
    ...
    /****************************************************************************/
    private Point getAbsolutePosition(Object component) {
        int x = 0, y = 0;
     ...
     */
    private void handleMouseEvent(int x, int y, int clickcount, boolean shiftdown, boolean controldown, boolean popuptrigger, int ibutton, int id,
            Object component, Object part) {
        if (id == MouseEvent.MOUSE_ENTERED) {
            setTimer(750L);
        } else if (id == MouseEvent.MOUSE_EXITED) {
            hideTip();
        ...
        if ("canvas".equals(classname)) {
            ...
            if (id == MouseEvent.MOUSE_PRESSED) {
                MouseEvent e = new MouseEvent(this, id, System.currentTimeMillis(), 0, x - pos.x, y - pos.y, clickcount, popuptrigger, ibutton);
                invoke(component, null, "press", e);
            }
            if (id == MouseEvent.MOUSE_DRAGGED) {
                MouseEvent e = new MouseEvent(this, id, System.currentTimeMillis(), 0, x - pos.x, y - pos.y, 0, false, ibutton);
                invoke(component, null, "drag", e);
            }
            if (id == MouseEvent.MOUSE_MOVED) {
                MouseEvent e = new MouseEvent(this, id, System.currentTimeMillis(), 0, x - pos.x, y - pos.y, 0, false, ibutton);
                invoke(component, null, "move", e);
            }
            ...
            if ((header != null) && (get(header, "action") != null)) {
                if ((insidepart != null) && (insidepart instanceof Object[]) && ("column".equals(getClass(insidepart)))) {
                    noScroll = true;
                    if ((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_PRESSED) || (id == MouseEvent.MOUSE_RELEASED)) {
                        if ((id == MouseEvent.MOUSE_RELEASED) && (mouseinside == component)) {
                            ...
                            while (column != null) {
                                if (column == insidepart) {
                                    ...
                                    if ((null == sort) || ("none".equals(sort)) || ("descent".equals(sort))) {
                                        sort = "ascent";
                                ...
                                setBoolean(column, "selected", (sort != null) && (!"none".equals(sort)));
                                column = get(column, ":next");
                            }
                    ...
                    for (Object item = get(component, ":comp"); item != null;) {
                        ...
                        if (my < (r.y + r.height)) {
                            ...
                            if ((id != MouseEvent.MOUSE_DRAGGED) || !getBoolean(item, "selected", false)) {
                                ...
                                if (clickcount == 2) {
                                    int mx = (x + port.x) - referencex;
                                    if (!invokeCell(component, item, mx)) {
            ...
            if ((id == MouseEvent.MOUSE_PRESSED) || (id == MouseEvent.MOUSE_RELEASED) || (id == MouseEvent.MOUSE_EXITED) || (id == MouseEvent.MOUSE_ENTERED)
                    || (id == MouseEvent.MOUSE_MOVED) || (id == MouseEvent.MOUSE_DRAGGED)) {
                Component bean = (Component) get(component, "bean");
                bean.dispatchEvent(new MouseEvent(this, id, System.currentTimeMillis(), modifiers, ((Integer) get(component, ":mousex")).intValue(),
                        ((Integer) get(component, ":mousey")).intValue(), clickcount, popuptrigger));
            }
        } else if (((clickcount < 2) && ("button".equals(classname))) || ("checkbox".equals(classname)) || ("togglebutton".equals(classname))) {
            if ((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_EXITED) || (id == MouseEvent.MOUSE_PRESSED) || (id == MouseEvent.MOUSE_RELEASED)) {
                if (id == MouseEvent.MOUSE_PRESSED) {
                    setFocus(component);
                }
                if (("button".equals(classname)) && ((mousepressed == null) || (mousepressed == component))
            ...
            processField(x, y, clickcount, id, component, false, ("passwordfield".equals(classname)), 0);
        } else if ("textarea".equals(classname)) {
            if (!processScroll(x, y, id, component, part)) {
            ...
            if ((id == MouseEvent.MOUSE_PRESSED) || (id == MouseEvent.MOUSE_DRAGGED)) {
                ...
                int newvalue = minimum
                        + (((horizontal ? (x - referencex) : (y - referencey)) * (maximum - minimum)) / ((horizontal ? bounds.width : bounds.height) - block)); // ...
                                                                                                                                                                // +0.5

                newvalue = Math.max(minimum, Math.min(newvalue, maximum));
                if (value != newvalue) {
                    setInteger(component, "value", newvalue, maximum + 1);
                    invoke(component, null, "action");
                }
                ...
                int newvalue = getInteger(component, "value", 50);
            ...
            if (part != null) {
                ...
                } else if (((id == MouseEvent.MOUSE_EXITED) || (id == DRAG_EXITED)) && getBoolean(part, "enabled", true)) {
            ...
            if ("header".equals(part)) {
                if (id == MouseEvent.MOUSE_PRESSED) {
                    ...
                    if ((bounds.x != mx) || (bounds.y != my)) {
                        ...
                        repaint(component, Math.min(bounds.x, mx), Math.min(bounds.y, my), bounds.width + Math.abs(mx - bounds.x),
                                            ...
                                            : (part == ":se") ? Cursor.SE_RESIZE_CURSOR
                                                    : (part == ":s") ? Cursor.S_RESIZE_CURSOR
                                                            : (part == ":sw") ? Cursor.SW_RESIZE_CURSOR
                                                                    : (part == ":w") ? Cursor.W_RESIZE_CURSOR : Cursor.NW_RESIZE_CURSOR));
                } else if (id == MouseEvent.MOUSE_EXITED) {
                    setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
                }
            }
    ...
    boolean invokeCell(Object table, Object row, int mx) {
        String classname;

        classname = getClass(table);
        if ("table" != classname) {
            return false;
        }
        ...
        Object header = get(table, "header");
        if (header == null) {
            return false;
        }
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            xcumul += w;
            if (xcumul > mx) {
                Object cell = getItem(row, i);
                return invoke(cell, null, "perform");
            }
        }
    ...
    /**
     * Calculate the given point in a component relative to the thinlet desktop and set as reference value
     *
     * @param component
     *            a widget
     * @param x
    ...
    private void select(Object component, Object row, boolean recursive, boolean shiftdown, boolean controldown) {
        String selection = getString(component, "selection", "single");
        Object lead = null;
        if (shiftdown && (!"single".equals(selection)) && ((lead = get(component, ":lead")) != null)) {
            extend(component, lead, row, recursive);
        } else {
     ...
     *
     */
    private void processField(int x, int y, int clickcount, int id, Object component, boolean multiline, boolean hidden, int left) {
        if (id == MouseEvent.MOUSE_PRESSED) {
            ...
            int oldcaretend = getInteger(component, "end", 0);
            int caretstart = getCaretLocation(component, mx, my, multiline, hidden);
            int caretend = caretstart;
            if (clickcount > 1) {
                String text = getString(component, "text", "");
                while ((caretstart > 0)
                        && ((clickcount == 2) ? Character.isLetterOrDigit(text.charAt(caretstart - 1)) : (text.charAt(caretstart - 1) != '\n'))) {
                    caretstart--;
                }
            ...
            if (caretend != oldcaretend) {
                invoke(component, null, "caret");
            }
            setFocus(component);
    ...
    private boolean processScroll(int x, int y, int id, Object component, Object part) {
        if (("up".equals(part)) || ("down".equals(part)) || ("left".equals(part)) || ("right".equals(part))) {
            if ((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_EXITED) || (id == MouseEvent.MOUSE_PRESSED) || (id == MouseEvent.MOUSE_RELEASED)) {
                if (id == MouseEvent.MOUSE_PRESSED) {
        ...
        } else if (("uptrack".equals(part)) || ("downtrack".equals(part)) || ("lefttrack".equals(part)) || ("righttrack".equals(part))) {
            if (id == MouseEvent.MOUSE_PRESSED) {
                if (processScroll(component, part)) {
                    setTimer(300L);
                }
    ...
    private boolean processScroll(Object component, Object part) {
        Rectangle view = getRectangle(component, ":view");
        Rectangle port = (("left".equals(part)) || ("up".equals(part))) ? null : getRectangle(component, ":port");
        int dx = 0;
        int dy = 0;
        if ("left".equals(part)) {
            dx = -10;
        ...
        return ((("left".equals(part)) || ("lefttrack".equals(part))) && (view.x > 0))
                || ((("right".equals(part)) || ("righttrack".equals(part))) && (view.x < (view.width - port.width)))
                || ((("up".equals(part)) || ("uptrack".equals(part))) && (view.y > 0))
                || ((("down".equals(part)) || ("downtrack".equals(part))) && (view.y < (view.height - port.height)));
    }
    ...
    private boolean processSpin(Object component, Object part) {
        ...
        try {
            ...
            if (("up".equals(part)) ? ((itext + step) <= getInteger(component, "maximum", Integer.MAX_VALUE))
                    : ((itext - step) >= getInteger(component, "minimum", Integer.MIN_VALUE))) {
                String value = String.valueOf(("up".equals(part)) ? (itext + step) : (itext - step));
    ...
    private boolean invoke(Object component, Object part, String event) {
        return invoke(component, part, event, null);
    }

    private boolean invoke(Object component, Object part, String event, Object first) {
        Object method = get(component, event);
        if (method != null) {
            invokeImpl(method, part, first);
            return true;
        }
        return false;
    }

    private void invokeImpl(Object method, Object part, Object first) {
    ...
    private boolean findComponent(Object component, int x, int y) {
        ...
        if ("combobox".equals(classname)) {
        ...
        } else if (("panel".equals(classname)) || ("desktop".equals(classname)) || ("dialog".equals(classname))) {
            if ("dialog".equals(classname)) {
                int titleheight = getInteger(component, ":titleheight", 0);
            ...
            if ((insidepart == null) && !findScroll(component, x, y)) {
                ...
                for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                    ...
                    if (("desktop".equals(classname)) && getBoolean(comp, "modal", false)) {
                        insidepart = "modal";
                        break;
                    } // && dialog
                }
                ...
                if (isResizable || hasAction) {
                    ...
                    if ((0 < x) && (x < port.width) && (0 < y) && (y < (port.y - 1))) {
                        ...
                        for (int i = 0; i < columnwidths.length; i++) {
                            ...
                            if (isResizable && (((x > ((left + width) - 4)) && (x < (left + width)))
                                    || ((i < (columnwidths.length - 1)) && (x >= (left + width)) && (x < (left + width + 4))))) {
     ...
     * @param y
     *            point y location
     * @return true if the point (x, y) is inside scroll-control area (scrollbars, corners, borders), false otherwise (vievport, header, or no scrollpane)
     */
    ...
    private void repaint(Object component, Object classname, Object part) {
        ...
        if ("combobox".equals(classname)) { // combobox down arrow
            ...
            if ("text".equals(part)) { // spinbox textfield content
                ...
                repaint(component, (b.x + b.width) - block, ("up".equals(part)) ? b.y : ((b.y + b.height) - (b.height / 2)), block, b.height / 2);
            }
        }
        // else if ("dialog".equals(classname)) {}
        // int titleheight = getInteger(component, ":titleheight", 0);
        ...
        // classname: ":combolist" "textarea" "list" "table" "tree"
            ...
            repaint(component, b.x + (("left".equals(part)) ? r.x : ((r.x + r.width) - block)), b.y + r.y, block, r.height);
        } else if (("up".equals(part)) || ("down".equals(part))) { // vertical scrollbar
            // button
            Rectangle r = getRectangle(component, ":vertical");
            repaint(component, b.x + r.x, b.y + (("up".equals(part)) ? r.y : ((r.y + r.height) - block)), r.width, block);
    ...
    /**
     * Requests that both the <i>Thinlet</i> component, and the given widget get the input focus
     *
     * @param component
     *            a focusable widget inside visible and enabled parents, and tabbedpane's selected tab
    ...
    /**
     * For the starting component search its parent direction for a focusable component, and then its next component (if not search backward from the
     * component).<br />
     * For its parent components check its first component, the current one, and its parent direction (backward search), or its parent, then next component
     * (forward direction).<br />
     * For the rest components check the next, then the first subcomponent direction, and finally check whether the component is focusable.
     */
    private Object getPreviousFocusable(Object component, Object block, boolean start, boolean upward, boolean backward, boolean outgo) {
        Object previous = null;
     ...
     *            check this widget
     * @param forced
     *            splitpane is also checked (e.g. false for tab navigating, and true for mouse selection or application request)
     * @return true if focusable, otherwise false
     */
    private boolean isFocusable(Object component, boolean forced) {
        String classname = getClass(component);
        if (("button".equals(classname)) || ("checkbox".equals(classname)) || ("togglebutton".equals(classname)) || ("canvas".equals(classname))
                || ("combobox".equals(classname)) || ("textfield".equals(classname)) || ("passwordfield".equals(classname)) || ("textarea".equals(classname))
                || ("spinbox".equals(classname)) || ("slider".equals(classname)) || ("list".equals(classname)) || ("table".equals(classname))
                || ("tree".equals(classname)) || ("tabbedpane".equals(classname)) || (forced && ("splitpane".equals(classname)))) {
     ...
     *
     */
    public static Object createImpl(String classname) {
    ...
    public Object getSelectedItem(Object component) {
        String classname = getClass(component);
        if (("combobox".equals(classname)) || ("tabbedpane".equals(classname))) {
            int index = getInteger(component, "selected", ("combobox".equals(classname)) ? -1 : 0);
     ...
     * @param index
     *            the position at which to insert the component, or -1 to insert the component at the end
     */
    ...
    /**
     * mnemonic (e.g. Alt-X): - check: label, button, checkbox, togglebutton, menubar menus, tabbedpane tabs - path: panel, desktop, dialog, splitpane
     * components, tabbedpane selected component accelerator (e.g. Ctrl-Shift-X, F4): - check: menuitem, checkboxmenuitem - path: see above, and menubar, and
     * menu items menubar F10: check menubar only button enter, escape: check button only
     *
     * @param component
     * @param parent
    ...
    private boolean checkMnemonic(Object component, boolean parent, Object checked, int keycode, int modifiers) {
        if ((component == null) || !getBoolean(component, "visible", true) || !getBoolean(component, "enabled", true)) { // + enabled comp
                                                                                                                         // in disabled
                                                                                                                         // parent
            return false;
        }
        ...
        if ("label".equals(classname)) {
            ...
            if (((modifiers == 0) && (((keycode == KeyEvent.VK_ENTER) && (get(component, "type") == "default"))
                    || ((keycode == KeyEvent.VK_ESCAPE) && (get(component, "type") == "cancel")))) || hasMnemonic(component, keycode, modifiers)) {
                invoke(component, null, "action");
                repaint(component);
                return true;
            }
        ...
        // check subcomponents
        if (("panel".equals(classname)) || ("desktop".equals(classname)) || ("dialog".equals(classname)) || ("splitpane".equals(classname))
        ...
        if (parent && (("dialog" != classname) || !getBoolean(component, "modal", false))) {
            if (checkMnemonic(getParent(component), true, ("tab".equals(classname)) ? checked : component, keycode, modifiers)) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Binds the specified key to the specified value, and stores in this component. <i>Null</i> value removes the property. Use the parameter tag in the xml
    ...
    public void putProperty(Object component, Object key, Object value) {
        ...
        if (value != null) {
            if (table == null) {
                set(component, ":bind", table = new HashMap<>());
            }
     ...
     *
     * @param path
     *            is relative to your thinlet instance or the classpath (if the path starts with an <i>/</i> character), or a full URL
    ...
    public Object parse(String path, Object handler) throws IOException {
        return parse(getInputStream(path), handler);
    }
    ...
    protected void parseXML(String path, Object handler) throws IOException {
        parse(getInputStream(path), 'S', handler);
    }

    protected void parseXML(InputStream inputstream, Object handler) throws IOException {
    ...
    protected Object parseDOM(String path, Object handler) throws IOException {
        return parse(getInputStream(path), 'D', handler);
    }

    protected Object parseDOM(InputStream inputstream, Object handler) throws IOException {
        return parse(inputstream, 'D', handler);
    }

    public static String getDOMAttribute(Object node, String key) {
        return (String) get(node, key.intern());
    }

    public static String getDOMText(Object node) {
        return (String) get(node, ":text");
    }

    public static int getDOMCount(Object node, String key) {
        return getItemCountImpl(node, key.intern());
    }

    public static Object getDOMNode(Object node, String key, int index) {
     ...
     *            a bundle for the next parsing or null to remove the current one
                ...
                if (c == '<') {
                    if ((c = reader.read()) == '/') { // endtag
                        if ((text.length() > 0) && (text.charAt(text.length() - 1) == ' ')) {
                            text.setLength(text.length() - 1); // trim last
                                                               // space
                        }
                        if (text.length() > 0) {
                            if (mode == 'D') { // DOM parser
                                ...
                                ((ThinSAXParser) handler).characters(new String(text.toString()));
                            }
                            text.setLength(0);
                        }
                        String tagname = (String) parentlist[2];
                        for (int i = 0; i < tagname.length(); i++) { // check
                                                                     // current
                                                                     // tag's
                                                                     // name
                        ...
                        if (c != '>') {
                            throw new IllegalArgumentException(tagname); // read
                                                                         // '>'
                        }
                        if (mode == 'S') {
                        ...
                        while (">/ \t\n\r".indexOf(c) == -1) { // to next
                                                               // whitespace or
                                                               // '/'
                            text.append((char) c);
                            if ((text.length() == 3) && (text.charAt(0) == '!') && (text.charAt(1) == '-') && (text.charAt(2) == '-')) { // comment
                                int m = 0;
                                while (true) { // read to '-->'
                                    c = reader.read();
                                    if (c == '-') {
                        ...
                        boolean pi = (text.charAt(0) == '?'); // processing
                                                              // instruction
                        ...
                        while (true) { // read attributes
                            ...
                            if (pi && (c == '?')) { // end of processing
                                ...
                                if (mode == 'S') { // SAX parser
                                    ((ThinSAXParser) handler).startElement((String) current, attributes);
                                    attributes = null;
                                }
                                ...
                                if (pi) {
                                    ...
                                    attributes.put(new String(key), new String(text.toString()));
                                }
                                // '<![CDATA[' ']]>'
                                text.setLength(0);
                                c = reader.read();
    ...
    public void finishParse(Object component, Object definition[], String value, Object root, Object handler) {
    ...
    private void addImpl(Object parent, Object component, int index) {
        ...
        String classname = getClass(component);
        if ((("combobox".equals(parentclass)) && ("choice".equals(classname))) || (("tabbedpane".equals(parentclass)) && ("tab".equals(classname)))
                || (("list".equals(parentclass)) && ("item".equals(classname))) || (("table".equals(parentclass)) && ("row".equals(classname)))
                || (("header".equals(parentclass)) && ("column".equals(classname))) || (("row".equals(parentclass)) && ("cell".equals(classname)))
                || ((("tree".equals(parentclass)) || ("node".equals(parentclass))) && ("node".equals(classname)))
                || (("menubar".equals(parentclass)) && ("menu".equals(classname)))
                || ((("menu".equals(parentclass)) || ("popupmenu".equals(parentclass))) && (("menu".equals(classname)) || ("menuitem".equals(classname))
                        || ("checkboxmenuitem".equals(classname)) || ("separator".equals(classname))))
                || ((("panel".equals(parentclass)) || ("desktop".equals(parentclass)) || ("splitpane".equals(parentclass)) || ("dialog".equals(parentclass))
                        || ("tab".equals(parentclass))) && instance(classname, "component") && (!"popupmenu".equals(classname)))) {
            insertItem(parent, ":comp", component, index);
            set(component, ":parent", parent);
        } else if ((("table".equals(parentclass)) && ("header".equals(classname))) || (("popupmenu".equals(classname)) && instance(parentclass, "component"))) {
    ...
    private List<Object> addAttribute(Object component, String key, String value, String encoding, List<Object> lasts) throws UnsupportedEncodingException {
        ...
        if ("string".equals(definition[0])) {
        ...
        } else if (("method".equals(definition[0])) || ("component".equals(definition[0]))) {
            if (lasts == null) {
                lasts = new ArrayList<>();
            }
            lasts.add(component);
            ...
            if (value.startsWith("#")) {
                ...
                StringTokenizer st = new StringTokenizer(value, " \r\n\t,");
                color = 0xff000000 | ((Integer.parseInt(st.nextToken()) & 0xff) << 16) | ((Integer.parseInt(st.nextToken()) & 0xff) << 8)
    ...
    public void setBoolean(Object component, String key, boolean value) {
        ...
        if (setBoolean(component, (String) definition[1], value, Boolean.TRUE.equals(definition[3]))) {
            update(component, definition[2]);
        }
    }
    ...
    /**
     * Set custom font on a component, use the other <code>setFont</code> method instead
     */
     ...
     * @param font
     *            custom font to use, or null to reset component to use default font
     */
    ...
    /**
     * Set custom color on a component. Notes: For "foreground" key, this sets the text color. For "background" key, on gradient-filled components (such as
     * tabs, buttons etc) this will result in a component filled with solid background color, and not a new gradient. Also, Color.brighter() will be used for
     * highlight, and Color.darker() will be used for pressed or not selected.
     *
     * @param component
     ...
     * @param color
     *            custom color to use, or null to reset component to use default color
     */
     ...
     * @param value
     *            the method name and parameters (e.g. <i>foo(this, this.text, mybutton, mybutton.enabled)</i> for <i>public void foo(Object component, String
     *            text, Object mybutton, boolean enabled)</i>)
     * @param root
     *            the search starting component for name components in the arguments
     * @param handler
    ...
    public void setMethod(Object component, String key, String value, Object root, Object handler, Class<?> first) {
        key = (String) getDefinition(getClass(component), key, "method")[1];
        Object[] method = getMethod(component, value, root, handler, first);
        set(component, key, method);
    }
    ...
    /**
     * @return an object list including as follows: - handler object, - method, - list of parameters including 3 values: - ("thinlet", null, null) for the
     *         single thinlet component, - (target component, null, null) for named widget as parameter, e.g. mybutton, - (target, parameter name, default
     *         value) for a widget's given property, e.g. mylabel.enabled, - ("item", null, null) for an item of the target component as parameter, e.g. tree
     *         node, - ("item", parameter name, default value) for the item's given property e.g. list item's text.
     */
    private Object[] getMethod(Object component, String value, Object root, Object handler) {
        return getMethod(component, value, root, handler, null);
    }

    private Object[] getMethod(Object component, String value, Object root, Object handler, Class<?> first) {

        StringTokenizer st = new StringTokenizer(value, "(, \r\n\t)");
        String methodname = st.nextToken();
        int n = st.countTokens();

        Object[] data = (first == null) ? new Object[2 + (3 * n)] : new Object[2 + (3 * (n + 1))];

        Class<?>[] parametertypes;
        int j = 0;

        if (first == null) {
            parametertypes = (n > 0) ? new Class[n] : null;
        } else {
            parametertypes = new Class[n + 1];
            parametertypes[j] = first;
            data[2 + (3 * j)] = "first";
            data[2 + (3 * j) + 1] = first;
            data[2 + (3 * j) + 2] = null;
            j++;
        ...
        for (int i = 0; i < n; i++, j++) {
            ...
            if ("thinlet".equals(arg)) {
                data[2 + (3 * j)] = "thinlet"; // the target component
                parametertypes[j] = Thinlet.class;
            } else if ((arg.length() > 1) && // constant string value
                    (arg.charAt(0) == '\'') && (arg.charAt(arg.length() - 1) == '\'')) {
                data[2 + (3 * j)] = "constant";
                data[2 + (3 * j) + 1] = new String(arg.substring(1, arg.length() - 1));
                ...
                } else if ((comp = find(root, compname)) != null) {

                    classname = getClass(comp);
                } else {
                    try { // maybe constant number
                        if (arg.regionMatches(true, arg.length() - 1, "F", 0, 1)) { // float
                            data[2 + (3 * j) + 1] = Float.valueOf(arg.substring(0, arg.length() - 1));
                            parametertypes[j] = Float.TYPE;
                        } else if (arg.regionMatches(true, arg.length() - 1, "L", 0, 1)) { // long
                            data[2 + (3 * j) + 1] = Long.valueOf(arg.substring(0, arg.length() - 1));
                            parametertypes[j] = Long.TYPE;
                        } else if (dot != -1) { // double
                            data[2 + (3 * j) + 1] = Double.valueOf(arg);
                            parametertypes[j] = Double.TYPE;
                        } else { // integer
                            data[2 + (3 * j) + 1] = Integer.valueOf(arg);
                ...
                data[2 + (3 * j)] = comp; // the target component
                if (dot == -1) {
                    parametertypes[j] = Object.class; // Widget.class
                } else {
                    Object[] definition = getDefinition(classname, arg.substring(dot + 1), null);
                    data[2 + (3 * j) + 1] = definition[1]; // parameter name, e.g.
                                                           // enabled
    ...
    private void update(Object component, Object mode) {
        ...
        while (component != null) {
            ...
            if (("layout".equals(mode)) || (("validate".equals(mode)) && (("list".equals(classname)) || ("table".equals(classname))
    ...
    private boolean setString(Object component, String key, String value, String defaultvalue) {
        if (allI18n && (langResource != null) && (("text".equals(key)) || ("tooltip".equals(key)))) {
    ...
    /**
     * Set current language resource bundle. This flushes all cached translated values, performs lazy loading of new values, and repaints the desktop. This
     * implementation allows applications to switch language resources on the fly, without rebuilding/reloading components that use them. <br />
     * The pseudo-code is as follows:
     *
     * <ul>
     * <li>if langResource and langResourceDefault are null, don't translate anything, no matter what other settings are. This behaviour provides compatibility
     * with previous versions.</li>
     * <li>if only langResourceDefault is set, use this when translation is required</li>
     * <li>if allI18n is set to true:
     * <ul>
     * <li>if property "i18n" on a component is missing, or set to "true", translate</li>
     * <li>if property "i18n" is present, and set to "false", do not translate</li>
     * </ul>
     * </li>
     * <li>if allI18n is set to false:
     * <ul>
     * <li>if property "i18n" on a component is missing, or set to "false", do not translate</li>
     ...
     * <ul>
     * <li>if no value is found, just return the original value of the property. Set a flag on component that prevents lookups in the future. This flag is
     * cleared when langResource is changed.</li>
     * </ul>
     * </li>
     * </ul>
     * </li>
     ...
     * </ul>
     *
     * If translated value is found successfully, it is cached in the component. This cache is gradually flushed when setLangResource is called. Cached value is
     * also flushed when setString() is called on a component.
     *
     * @param res
     *            resource bundle containing localized texts for "text" and "tooltip"
     */
     ...
     *
     * @param res
     *            resource bundle containing default localized texts for "text" and "tooltip"
     */
    ...
    /**
     * Sets the default behaviour of internationalization code. If set to "true", try to translate all components' "text" and "tooltip" values, unless
     * explicitly prohibited by setting <code>i18n="false"</code> on a specific component. If set to "false", do not translate unless explicitly requested by
     * setting <code>i18n="true"</code> on a specific component. <br />
     * Default value is "false", to provide backwards compatibility.
     *
     * @param val
     *            if "true", translate by default; if "false", do not translate by default.
     */
    ...
    private String getI18NString(Object component, String key, String text) { // for
                                                                              // I18N
        if (allI18n && (langResource != null) && (("text".equals(key)) || ("tooltip".equals(key))) && getBoolean(component, "i18n", true)) {
     ...
     *
     */
    protected void setRectangle(Object component, String key, int x, int y, int width, int height) {
     ...
     *
     * @param path
     *            is relative to your thinlet instance or the classpath (if the path starts with <i>'/'</i> character), or a full URL
    ...
    /**
     * Creates an image from the specified resource. To speed up loading the same images use a cache (a simple Map). And flush the resources being used by an
     * image when you won't use it henceforward
     *
     * @param path
     *            is relative to your thinlet instance or the classpath, or an URL
     * @param preload
     *            waits for the whole image if true, starts loading (and repaints, and updates the layout) only when required (painted, or size requested) if
     *            false
     * @return the loaded image or null
     */
    ...
    /**
     * This method is called by the FrameLauncher if the window was closing, or AppletLauncher's destroy method. Overwrite it to e.g. save the application
     * changes.
     *
    ...
    static {
        ...
        dtd = new Object[] { "component", null,
                new Object[][] { { "string", "name", null, null }, { "boolean", "enabled", "paint", Boolean.TRUE },
                        { "boolean", "visible", "parent", Boolean.TRUE }, { "boolean", "i18n", "validate", Boolean.FALSE }, // for
                                                                                                                            // I18N
                        { "string", "tooltip", null, null }, { "font", "font", "validate", null }, { "color", "foreground", "paint", null },
                        { "color", "background", "paint", null }, { "integer", "width", "validate", integer0 }, { "integer", "height", "validate", integer0 },
                        ...
                        { "choice", "valign", "validate", new String[] { "fill", "center", "top", "bottom" } }, { "rectangle", "bounds", "paint", null },
                        // component class String null*
                        // parent Object null
                        // (bounds) Rectangle 0 0 0 0
                        { "property", "property", null, null }, { "method", "init" }, { "method", "focuslost" }, { "method", "focusgained" } },
                "label", "component",
                new Object[][] { { "string", "text", "validate", null }, { "icon", "icon", "validate", null },
                        { "choice", "alignment", "validate", leftcenterright }, { "integer", "mnemonic", "paint", integer_1 },
                        { "component", "for", null, null } },
                "button", "label",
                new Object[][] { { "choice", "alignment", "validate", new String[] { "center", "left", "right" } }, { "method", "action" },
                        { "choice", "type", "paint", new String[] { "normal", "default", "cancel", "link" } } },
                "checkbox", "label", new Object[][] { { "boolean", "selected", "paint", Boolean.FALSE }, // ...group
                        { "string", "group", "paint", null }, // ...group
                        { "method", "action" } },
                "togglebutton", "checkbox", null, "combobox", "textfield",
                new Object[][] { { "icon", "icon", "validate", null }, { "integer", "selected", "layout", integer_1 } }, "choice", null,
                new Object[][] { { "string", "name", null, null }, { "boolean", "enabled", "paint", Boolean.TRUE },
                        { "boolean", "i18n", "validate", Boolean.FALSE }, // for
                                                                          // I18N
                        { "string", "text", "parent", null }, { "icon", "icon", "parent", null }, { "choice", "alignment", "parent", leftcenterright },
                        { "string", "tooltip", null, null }, { "font", "font", "validate", null }, { "color", "foreground", "paint", null },
                        { "color", "background", "paint", null }, { "property", "property", null, null } },
                "textfield", "component",
                new Object[][] { { "string", "text", "layout", "" }, { "integer", "columns", "validate", integer0 },
                        { "boolean", "editable", "paint", Boolean.TRUE }, { "integer", "start", "layout", integer0 }, { "integer", "end", "layout", integer0 },
                        { "method", "action" }, { "method", "insert" }, { "method", "remove" }, { "method", "caret" }, { "method", "perform" } },
                "passwordfield", "textfield", null, "textarea", "textfield",
                ...
                "tab", "choice", new Object[][] { { "integer", "mnemonic", "paint", integer_1 } }, "panel", "component",
                new Object[][] { { "integer", "columns", "validate", integer0 }, { "integer", "top", "validate", integer0 },
                        ...
                        { "icon", "icon", "validate", null }, { "boolean", "border", "validate", Boolean.FALSE },
                        { "boolean", "scrollable", "validate", Boolean.FALSE } },
                "desktop", "component", null, "dialog", "panel",
                new Object[][] { { "boolean", "modal", null, Boolean.FALSE }, { "boolean", "resizable", null, Boolean.FALSE }, { "method", "close" },
                        { "boolean", "maximizable", "paint", Boolean.FALSE }, { "boolean", "iconifiable", "paint", Boolean.FALSE } },
                "spinbox", "textfield",
                new Object[][] { { "integer", "minimum", null, Integer.valueOf(Integer.MIN_VALUE) },
                        { "integer", "maximum", null, Integer.valueOf(Integer.MAX_VALUE) }, { "integer", "step", null, integer1 },
                        { "integer", "value", null, integer0 } }, // == text?
                                                                  // deprecated
                "progressbar", "component", new Object[][] { { "choice", "orientation", "validate", orientation }, { "integer", "minimum", "paint", integer0 }, // ...checkvalue
                        { "integer", "maximum", "paint", Integer.valueOf(100) }, { "integer", "value", "paint", integer0 } },
                // change stringpainted
                "slider", "progressbar",
                new Object[][] { { "integer", "unit", null, Integer.valueOf(5) }, { "integer", "block", null, Integer.valueOf(25) }, { "method", "perform" },
                        { "method", "action" } },
                // minor/majortickspacing
                // inverted
                // labelincrement labelstart
                "splitpane", "component",
                new Object[][] { { "choice", "orientation", "validate", orientation }, { "integer", "divider", "layout", integer_1 } }, "list", "component",
                new Object[][] { { "choice", "selection", "paint", selections }, { "method", "action" }, { "method", "perform" },
                        { "boolean", "line", "validate", Boolean.TRUE } },
                "item", "choice", new Object[][] { { "boolean", "selected", null, Boolean.FALSE } }, "table", "list", new Object[][] {
                /*
                 * { "choice", "selection", new String[] { "singlerow",
                 * "rowinterval", "multiplerow", "cell", "cellinterval",
                 * "singlecolumn", "columninterval", "multiplecolumn" } }
                 */ }, "header", null, new Object[][] { { "method", "action" }, { "boolean", "resizable", null, Boolean.FALSE } },
                // reordering allowed
                ...
                // editing row/column
                "column", "choice",
                new Object[][] { { "integer", "width", null, Integer.valueOf(DEFAULT_COLUMN_WIDTH) },
                        { "choice", "sort", null, new String[] { "none", "ascent", "descent" } },
                        { "choice", "datatype", null, new String[] { "text", "numeric" } }, { "boolean", "selected", null, Boolean.FALSE } },
                "row", null, new Object[][] { { "boolean", "selected", null, Boolean.FALSE } }, "cell", "choice", new Object[][] { { "method", "perform" } },
                "tree", "list", new Object[][] { { "boolean", "angle", null, Boolean.FALSE }, { "method", "expand" }, { "method", "collapse" } }, "node",
                "choice", new Object[][] { { "boolean", "selected", null, Boolean.FALSE }, { "boolean", "expanded", null, Boolean.TRUE } }, "separator",
                "component", null, "menubar", "component", new Object[][] { { "choice", "placement", "validate", new String[] { "top", "bottom" } } }, "menu",
                "choice", new Object[][] { { "integer", "mnemonic", "paint", integer_1 } }, "menuitem", "choice",
                new Object[][] { { "keystroke", "accelerator", null, null }, { "method", "action" }, { "integer", "mnemonic", "paint", integer_1 } },
                "checkboxmenuitem", "menuitem", new Object[][] { { "boolean", "selected", "paint", Boolean.FALSE }, // ...group
                        { "string", "group", "paint", null } }, // ...group
                "popupmenu", "component", new Object[][] { { "method", "menushown" } }, // Post menu:
                                                                                        // Shift+F10
                "canvas", "component", new Object[][] { { "method", "size" }, { "method", "draw" }, { "method", "press" }, { "method", "move" },
                        { "method", "drag" }, { "method", "release" }, { "method", "key" } } };
    }
}
