...
if ("label" == classname) {
	...
	if ((tooltipowner != null) && (component == content)) {
		...
		paintRect(g, r.x, r.y, r.width, r.height,
			c_border, c_tooltip, true, true, true, true, true);
	...
	paintRect(g, 0, 0, horizontal ? length : bounds.width,
		...
		fg, true, true, horizontal, !horizontal, true);
	...
	paintRect(g, horizontal ? length : 0, horizontal ? 0 : length,
		...
		enabled ? c_border : c_disable, bg , true, true, true, true, true);
}
		...
		if ((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_MOVED) ||
			...
			if (id == MouseEvent.MOUSE_ENTERED) {
				if (mousepressed == null) {
					...
					handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger,ibutton,
			...
			//
				...
				if ((control == (id == KeyEvent.KEY_PRESSED)) &&
					processKeyPress((popupowner != null) ? popupowner : focusowner,
						ke.isShiftDown(), controldown , ke.getModifiers(),
		...
		for (int c = reader.read(); c != -1;) {
			if (c == '<') {
				if ((c = reader.read()) == '/') { //endtag
					...
					if (mode == 'S') { ((ThinSAXParser)handler).endElement(); } // SAX parser
					if (parentlist[0] == null) {
						reader.close();
						finishParse(methods, current, handler);
						return current;
					...
					while (true) { // read attributes
						...
						if (pi && (c == '?')) { // end of processing instruction
							...
							if (mode == 'S') { // SAX parser
								((ThinSAXParser)handler).startElement((String) current, attributelist); attributelist = null;
							}
						}
...
public void finishParse(Object component, Object definition[], String value,
...
public void setComponent(Object component, String key, Component bean) {
	...
	public Object getWidget(Object component, String key) {
		...
		else if ("header".equals(key)) { return get(component, "header");} else {
...
for (int i = 0; i < n; i++,j++) {
	...
	if ("thinlet".equals(arg)) {
		...
		if ("item".equals(compname)) {
			...
			else if ("tabbedpane" == parentclass) { classname = "tab"; } else {
...
// widget's name not found nor constant
    throw new IllegalArgumentException("unknown " + arg);                        }
		...
		if (dot == -1) {
			...
			data[2 + 3 * j + 1] = definition[1]; // parameter name, e.g. enabled
			data[2 + 3 * j + 2] = definition[3]; // default value, e.g. Boolean.TRUE
	...
	static {
		...
		dtd = new Object[] {
			...
			"column", "choice", new Object[][] {
				...
				{ "choice", "datatype", null, new String[] { "text", "numeric" } },
				{ "boolean", "selected", null, Boolean.FALSE } },
			"row", null, new Object[][] {
				{ "boolean", "selected", null, Boolean.FALSE } },
