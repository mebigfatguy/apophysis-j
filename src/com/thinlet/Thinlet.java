...
public class Thinlet extends Container implements Runnable, Serializable {
	...
	}
	{
		...
		setColors(0xe6e6e6, 0x000000, 0xffffff, 0x909090, 0xb0b0b0, 0xededed,
				0xb9b9b9, 0x89899a, 0xc5c5dd, DEFAULT_RESIZE_BORDER_COLOR);

		// disable global focus-manager for this component in 1.4
		if (MOUSE_WHEEL != 0) {
			try {
				getClass().getMethod("setFocusTraversalKeysEnabled",
						new Class[] { Boolean.TYPE }).invoke(this,
	...
	public void setColors(int background, int text, int textbackground,
		...
		setColors(background, text, textbackground, border, disable, hover,
				press, focus, select, DEFAULT_RESIZE_BORDER_COLOR);
	}
	...
	protected void doLayout(Object component) {
		...
		if ("combobox" == classname) {
			...
			layoutScroll(component, width + 2, height - fm.getLeading() + 2, 0,
	...
	private void layoutField(Object component, int dw, boolean hidden, int left) {
		...
		FontMetrics fm = getFontMetrics((currentfont != null) ? currentfont
				: font);
		int caret = hidden ? (fm.charWidth('*') * end) : fm.stringWidth(text
				.substring(0, end));
		if (off > caret) {
			off = caret;
		...
		off = Math.max(
				0,
				Math.min(off, (hidden ? (fm.charWidth('*') * text.length())
						: fm.stringWidth(text)) - width + 4));
		if (off != offset) {
	...
	private Dimension getPreferredSize(Object component) {
		...
		if ("textarea" == classname) {
			...
			return new Dimension(
					((columns > 0) ? (columns * fm.charWidth('e') + 2) : 76)
							+ 2 + block, ((rows > 0) ? (rows * fm.getHeight()
							- fm.getLeading() + 2) : 76)
							+ 2 + block);
		}
	...
	private Dimension getFieldSize(Object component) {
		...
		return new Dimension(((columns > 0) ? (columns * fm.charWidth('e'))
				: 76) + 4, fm.getAscent() + fm.getDescent() + 4); // fm.stringWidth(text)
	}
	...
	public void paint(Graphics g) {
		if (TXT_AA != null) { // set antialiasing
			try {
				if (renderinghint == null) {
					renderinghint = g.getClass().getMethod(
							"setRenderingHint",
							new Class[] { TXT_AA[0].getClass().getSuperclass(),
									Object.class });
				}
		...
		if (hgradient == null) {
			...
			hgradient = createImage(new MemoryImageSource(block, block, pix[0],
					0, block));
			vgradient = createImage(new MemoryImageSource(block, block, pix[1],
					0, block));
		}
	...
	private void paint(Graphics g, int clipx, int clipy, int clipwidth,
		...
		if ("label" == classname) {
			...
			if (getBoolean(component, "editable", true)) {
				...
				if (icon != null) {
					g.drawImage(icon, 2,
							(bounds.height - icon.getHeight(this)) / 2, this);
				}
			...
			if ("dialog" == classname) {
				...
				paintRect(g, 0, 3 + titleheight, bounds.width, bounds.height
						...
						true, true, true); // lower part excluding titlebar
			...
			if ((tooltipowner != null) && (component == content)) {
				...
				paintRect(g, r.x, r.y, r.width, r.height, c_border, c_tooltip,
			...
			paintRect(g, 0, 0, horizontal ? length : bounds.width,
					horizontal ? bounds.height : length, enabled ? c_border
							: c_disable, fg, true, true, horizontal,
					!horizontal, true);
			...
			if ("table".equals(classname)) {
				...
				if (header != null) {
					...
					for (int i = 0; i < columnwidths.length; i++) {
						...
						// or disabled (d)
						paint(column, x - view.x, 0, width, port.y - 1, g,
								clipx, clipy, clipwidth, clipheight, true,
								true, false, lastcolumn, 1, 1, 0, 0, false,
								enabled ? (insidepart == column ? 'h' : 'g')
										: 'd', "left", false, false);
	...
	/**
	 * @param column
	 *            the column for which we like to set its width to its current
	 *            smartwidth
	 */
	private void setSmartWidth(Object column) {
		...
		setInteger(column, "width",
				(((int[]) get(getParent(header), PROPERTY_SMARTWIDTHS))[index]));
	}
				...
				if (clipy >= r.y + r.height + iline) {
					if (angle) {
						...
						if (nodebelow != null) { // and the next node is bellow
							...
							int x = r.x - block / 2;
							g.drawLine(x, r.y, x,
									getRectangle(nodebelow, "bounds").y);
						}
					}
				...
				if ("table" != classname) { // list or tree
					...
					for (Object cell = get(item, ":comp"); cell != null; cell = get(
						...
						if ((header != null) && getBoolean(header, "resizable")) {
							...
							if (text != null) {
								smartWidth += g.getFontMetrics().stringWidth(
										text);
							}
							if (icon != null) {
								smartWidth += icon.getWidth(this)
										+ (text != null ? 2 : 0);
							}
							int[] smartWidths = (int[]) get(component,
									PROPERTY_SMARTWIDTHS);
	...
	private void paintRect(Graphics g, int x, int y, int width, int height,
			Color border, Color bg, boolean top, boolean left, boolean bottom,
		...
		if (horizontal) {
			...
			for (int i = 0; i < width; i += block) {
				g.drawImage(hgradient, x + i,
						...
						0, 0, Math.min(block, width - i) + evm,
			...
			for (int i = 0; i < height; i += block) {
				g.drawImage(vgradient, (width > block) ? (x + width - block)
						: x, y + i, x + width + evm,
						y + Math.min(i + block, height) + evm, 0, 0,
						Math.min(block, width) + evm,
						Math.min(block, height - i) + evm, null);
			}
		}
	}
		...
		if (text != null) {
			...
			if (mnemonic) {
				...
				if ((imnemonic != -1) && (imnemonic < text.length())) {
					...
					g.drawLine(mx, ty + 1,
							mx + fm.charWidth(text.charAt(imnemonic)), ty + 1);
				}
			}
			...
			if (id == MouseEvent.MOUSE_ENTERED) {
				if (mousepressed == null) {
					...
					handleMouseEvent(x, y, clickcount, shiftdown, controldown,
							popuptrigger, ibutton, MouseEvent.MOUSE_ENTERED,
				...
				if ((previnside == mouseinside) && (prevpart == insidepart)) {
					handleMouseEvent(x, y, clickcount, shiftdown, controldown,
							popuptrigger, ibutton, MouseEvent.MOUSE_MOVED,
							mouseinside, insidepart);
				} else {
					handleMouseEvent(x, y, clickcount, shiftdown, controldown,
							popuptrigger, ibutton, MouseEvent.MOUSE_EXITED,
							previnside, prevpart);
					handleMouseEvent(x, y, clickcount, shiftdown, controldown,
							popuptrigger, ibutton, MouseEvent.MOUSE_ENTERED,
							mouseinside, insidepart);
				...
				handleMouseEvent(x, y, clickcount, shiftdown, controldown,
						popuptrigger, ibutton, MouseEvent.MOUSE_PRESSED,
				...
				if (wasin && !isin) {
					...
					handleMouseEvent(x, y, clickcount, shiftdown, controldown,
							popuptrigger, ibutton, DRAG_EXITED, previnside,
							prevpart);
				}
				if (isin && !wasin) {
					handleMouseEvent(x, y, clickcount, shiftdown, controldown,
					...
					handleMouseEvent(x, y, clickcount, shiftdown, controldown,
							popuptrigger, ibutton, DRAG_ENTERED, mouseinside,
							insidepart);
				}
				if (isin == wasin) {
					handleMouseEvent(x, y, clickcount, shiftdown, controldown,
							popuptrigger, ibutton, MouseEvent.MOUSE_DRAGGED,
							mousepressed, pressedpart);
				}
				...
				handleMouseEvent(x, y, clickcount, shiftdown, controldown,
						popuptrigger, ibutton, MouseEvent.MOUSE_RELEASED,
			...
			if (focusinside && ((popupowner != null) || (focusowner != null))) {
				...
				if ((control == (id == KeyEvent.KEY_PRESSED))
						&& processKeyPress((popupowner != null) ? popupowner
								: focusowner, ke.isShiftDown(), controldown,
								ke.getModifiers(), control ? 0 : keychar,
								keycode)) {
					ke.consume();
				} else if ((keycode == KeyEvent.VK_TAB)
						|| ((keycode == KeyEvent.VK_F6) && (ke.isAltDown() || ke
								.isControlDown()))) {
					...
					for (Object splitpane = focusowner; splitpane != null; splitpane = getParent(splitpane)) {
						...
						&& checkMnemonic(focusowner, true, null,
	...
	private void checkLocation() {
		...
		handleMouseEvent(mousex, mousex, 1, false, false, false, 0,
	...
	private void handleMouseEvent(int x, int y, int clickcount,
			...
			int ibutton, int id, Object component, Object part) {
		if (id == MouseEvent.MOUSE_ENTERED) {
			setTimer(750L);
		} else if (id == MouseEvent.MOUSE_EXITED) {
			hideTip();
		...
		if ("canvas" == classname) {
			...
			if (id == MouseEvent.MOUSE_PRESSED) {
				MouseEvent e = new MouseEvent(this, id,
						System.currentTimeMillis(), 0, x - pos.x, y - pos.y,
						clickcount, popuptrigger, ibutton);
				invoke(component, null, "press", e);
			}
			if (id == MouseEvent.MOUSE_DRAGGED) {
				MouseEvent e = new MouseEvent(this, id,
						System.currentTimeMillis(), 0, x - pos.x, y - pos.y, 0,
						false, ibutton);
			...
			if (editable && (part == null)) { // textfield area
				Image icon = null;
				int left = ((id == MouseEvent.MOUSE_PRESSED) && ((icon = getIcon(
			...
			if (part == "header") {
				if (id == MouseEvent.MOUSE_PRESSED) {
					...
					int mx = Math.max(
							0,
							Math.min(x - referencex, parents.width
									- bounds.width));
					int my = Math.max(
							0,
							Math.min(y - referencey, parents.height
									- bounds.height));
	...
	private boolean findComponent(Object component, int x, int y) {
		...
		if ("combobox" == classname) {
			if (getBoolean(component, "editable", true)
				...
				Image icon = getIcon(component, "icon", null);
				insidepart = ((icon != null) && (x <= 2 + icon.getWidth(this))) ? "icon"
						: null;
			} else {
				insidepart = "down";
			}
	 ...
	 * Referenced by DOM
	 */
	protected void insertItem(Object parent, Object key, Object component,
			int index) {
	...
	protected Object parse(InputStream inputstream, Object handler)
			throws IOException {
	...
	protected void parseXML(InputStream inputstream, Object handler)
			throws IOException {
	...
	protected Object parseDOM(InputStream inputstream, Object handler)
			throws IOException {
	...
	/*
	 * private Object parse(InputStream inputstream, boolean validate, boolean
	 * dom, Object handler) throws IOException { Reader reader = new
	 * BufferedReader(new InputStreamReader(inputstream)); try { Object[]
	 ...
	 * 1); } if (!validate) { if (dom) { set(current, ":text", text.toString());
	 * } else { characters(text.toString()); } } // else { //addContent(current,
	 * text.toString()); //} text.setLength(0); } String tagname = (String)
	 * parentlist[2]; //getClass(current); for (int i = 0; i < tagname.length();
	 * i++) { // current-tag if ((c = reader.read()) != tagname.charAt(i)) {
	 * throw new IllegalArgumentException(tagname); } } while
	 * (" \t\n\r".indexOf(c = reader.read()) != -1); // whitespace if (c != '>')
	 * throw new IllegalArgumentException(); // '>' c = reader.read(); if
	 * (!validate && !dom) { endElement(); } if (parentlist[0] == null) {
	 * reader.close(); finishParse(methods, current, handler); return current; }
	 * current = parentlist[0]; parentlist = (Object[]) parentlist[1]; } else if
	 * (c == '!') { // DOCTYPE while ((c = reader.read()) != '>'); //+(-1) }
	 * else if (c == '?') { // Processing Instructions boolean question = false;
	 * // read until '?>' } else { //start or standalone tag text.setLength(0);
	 * boolean iscomment = false; while (">/ \t\n\r".indexOf(c) == -1) {
	 * text.append((char) c); if ((text.length() == 3) && (text.charAt(0) ==
	 * '!') && (text.charAt(1) == '-') && (text.charAt(2) == '-')) { int m = 0;
	 * while (true) { c = reader.read(); if (c == '-') { m++; } else if ((c ==
	 * '>') && (m >= 2)) { break; } else { m = 0; } } iscomment = true; } c =
	 * reader.read(); } if (iscomment) { continue; } String tagname =
	 * text.toString(); parentlist = new Object[] { current, parentlist, tagname
	 * }; if (validate) { current = (current != null) ? addElement(current,
	 * tagname) : create(tagname); } else { if (dom) { Object parent = current;
	 * current = createImpl(tagname = tagname.intern()); if (parent != null) {
	 * insertItem(parent, tagname, current, -1); //set(current, ":parent",
	 * parent); } } else { current = tagname; } } text.setLength(0); while
	 * (true) { boolean whitespace = false; while (" \t\n\r".indexOf(c) != -1) {
	 * c = reader.read(); whitespace = true; } if (c == '>') { if (!validate &&
	 * !dom) { startElement((String) current, attributelist); attributelist =
	 * null; } c = reader.read(); break; } else if (c == '/') { if ((c =
	 * reader.read()) != '>') { throw new IllegalArgumentException(); // '>' }
	 * if (!validate && !dom) { startElement((String) current, attributelist);
	 * attributelist = null; endElement(); } if (parentlist[0] == null) {
	 * reader.close(); finishParse(methods, current, handler); return current; }
	 * current = parentlist[0]; parentlist = (Object[]) parentlist[1]; c =
	 * reader.read(); break; } else if (whitespace) { while
	 * ("= \t\n\r".indexOf(c) == -1) { text.append((char) c); c = reader.read();
	 * } String key = text.toString(); text.setLength(0); while
	 * (" \t\n\r".indexOf(c) != -1) c = reader.read(); if (c != '=') throw new
	 * IllegalArgumentException(); while (" \t\n\r".indexOf(c = reader.read())
	 * != -1); char quote = (char) c; if ((c != '\"') && (c != '\'')) throw new
	 * IllegalArgumentException(); while (quote != (c = reader.read())) { if (c
	 * == '&') { StringBuffer eb = new StringBuffer(); while (';' != (c =
	 * reader.read())) { eb.append((char) c); } String entity = eb.toString();
	 * if ("lt".equals(entity)) { text.append('<'); } else if
	 * ("gt".equals(entity)) { text.append('>'); } else if
	 * ("amp".equals(entity)) { text.append('&'); } else if
	 * ("quot".equals(entity)) { text.append('"'); } else if
	 * ("apos".equals(entity)) { text.append('\''); } else if
	 * (entity.startsWith("#")) { boolean hexa = (entity.charAt(1) == 'x');
	 * text.append((char) Integer.parseInt(entity.substring(hexa ? 2 : 1), hexa
	 * ? 16 : 10)); } else throw new IllegalArgumentException("unknown " +
	 * "entity " + entity); } else text.append((char) c); } if (validate) {
	 * addAttribute(current, key, text.toString(), methods); } else { if (dom) {
	 * set(current, key.intern(), text.toString()); } else { if (attributelist
	 * == null) { attributelist = new Hashtable(); } attributelist.put(key,
	 * text.toString()); } } //'<![CDATA[' ']]>' text.setLength(0); c =
	 * reader.read(); } else throw new IllegalArgumentException(); } } } else {
	 * if (" \t\n\r".indexOf(c) != -1) { if ((text.length() > 0) &&
	 * (text.charAt(text.length() - 1) != ' ')) { text.append(' '); } } else {
	 * text.append((char) c); } c = reader.read(); } } throw new
	 * IllegalArgumentException(); } finally { if (reader != null) {
	 * reader.close(); } } }
	 */
					...
					if ((c = reader.read()) == '/') { // endtag
						...
						if (!pi) { // tagname is available
							...
							if (mode == 'T') { // GUI parser
								current = (current != null) ? addElement(
										current, tagname) : create(tagname);
							...
							if (pi && (c == '?')) { // end of processing
								...
								if (pi) {
									...
									methods = addAttribute(current, key,
											text.toString(), encoding, methods);
								} else if (mode == 'D') { // DOM parser
	...
	/**
	 * Set custom font on a component, use the other <code>setFont</code> method
	 * instead
	 */
			...
			try {
				...
				while (st.hasMoreTokens()) {
					...
					try {
						...
						keycode = KeyEvent.class.getField("VK_" + token)
								.getInt(null);
					}
				}
	 ...
	 *         - list of parameters including 3 values: - ("thinlet", null,
	 *         null) for the single thinlet component, - (target component,
	 *         null, null) for named widget as parameter, e.g. mybutton, -
	 *         (target, parameter name, default value) for a widget's given
	 *         property, e.g. mylabel.enabled, - ("item", null, null) for an
	 *         item of the target component as parameter, e.g. tree node, -
	 *         ("item", parameter name, default value) for the item's given
	 *         property e.g. list item's text.
	 */
	...
	private Object[] getMethod(Object component, String value, Object root,
		...
		for (int i = 0; i < n; i++, j++) {
			...
			if ("thinlet".equals(arg)) {
				...
				data[2 + 3 * j] = "constant";
				data[2 + 3 * j + 1] = new String(arg.substring(1,
						arg.length() - 1));
				...
				if ("item".equals(compname)) {
					...
					try { // maybe constant number
						if (arg.regionMatches(true, arg.length() - 1, "F", 0, 1)) { // float
							data[2 + 3 * j + 1] = Float.valueOf(arg.substring(
									0, arg.length() - 1));
							parametertypes[j] = Float.TYPE;
						} else if (arg.regionMatches(true, arg.length() - 1,
								"L", 0, 1)) { // long
							data[2 + 3 * j + 1] = Long.valueOf(arg.substring(0,
									arg.length() - 1));
				...
				if (dot == -1) {
					...
					Object[] definition = getDefinition(classname,
							arg.substring(dot + 1), null);
	 ...
	 * <ul>
	 * <li>if no value is found, just return the original value of the property.
	 * Set a flag on component that prevents lookups in the future. This flag is
	 ...
	 *
	 */
	protected void setRectangle(Object component, String key, int x, int y,
			int width, int height) {
	...
	static {
		...
		dtd = new Object[] {
				...
				"canvas",
				"component",
				new Object[][] { { "method", "size" }, { "method", "draw" },
						{ "method", "press" }, { "method", "move" },
						{ "method", "drag" }, { "method", "release" },
						{ "method", "key" } } };
	}
}
