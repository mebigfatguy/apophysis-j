...
public class Options extends MyThinlet implements Constants {
    ...
    Options(String title, String xmlfile, int width, int height) throws Exception {
    ...
    void buildVariationList() {
        ...
        for (int i = 0; i < nv; i++) {
            svar[i] = new SortableVariation(XForm.getVariation(i).getName(), i, XForm.getVariation(i).getGroup());
        }
    ...
    public void setVisible(boolean visible) {
        ...
        if (visible) {
            ...
            setBoolean(find("chkConfirmStopRender"), "selected", Global.confirmStopRender);

            setBoolean(find("chkOldPaletteFormat"), "selected", Global.oldPaletteFormat);

            setBoolean(find("chkShowRenderStats"), "selected", Global.showRenderStats);

            setBoolean(find("chkPlaysound"), "selected", Global.playSoundOnRenderComplete);
            ...
            setString(find("txtSampleDensity"), "text", "" + Global.defSampleDensity);
            ...
            setString(find("txtMediumQuality"), "text", "" + Global.prevMediumQuality);
            setString(find("txtHighQuality"), "text", "" + Global.prevHighQuality);

            // public static int gridColor1;
            // public static int gridColor2;

            setColor(find("btnBackgroundColor"), "background", new Color(Global.editorBkgColor));
            setColor(find("btnReferenceColor"), "background", new Color(Global.referenceTriangleColor));
            setColor(find("btnHelperColor"), "background", new Color(Global.helpersColor));
            setColor(find("btnGridColor1"), "background", new Color(Global.gridColor1));
            setColor(find("btnGridColor2"), "background", new Color(Global.gridColor2));

            setBoolean(find("chkUseXFormColor"), "selected", Global.useTransformColors);
            setBoolean(find("chkHelpers"), "selected", Global.helpersEnabled);
            setBoolean(find("chkShowAllXforms"), "selected", Global.showAllXforms);

            setBoolean(find("referencemode" + Global.referenceMode), "selected", true);
            ...
            for (int i = 1; i < 5; i++) {
                if (Global.randGradient == i) {
                    setBoolean(find("grpGradient" + Global.randGradient), "selected", true);
    ...
    public void btnRendererClick() {
        ...
        Global.opendialog = new OpenDialog(this, Global.browserPath, task);
        Global.opendialog.addFilter("Renderer (flam3-render*)", "flam3-render*");
        Global.opendialog.setVisible(true);
    ...
    public void btnParameterClick() {
        ...
        Global.opendialog.addFilter("Apophysis 1.0 files (*.apo,*.fla)", "*.apo;*.fla");
        Global.opendialog.setVisible(true);
    ...
    public void btnOKClick() {
        ...
        Global.confirmStopRender = getBoolean(find("chkConfirmStopRender"), "selected");
        Global.oldPaletteFormat = getBoolean(find("chkOldPaletteFormat"), "selected");

        Global.showRenderStats = getBoolean(find("chkShowRenderStats"), "selected");
        Global.playSoundOnRenderComplete = getBoolean(find("chkPlaysound"), "selected");
        Global.renderCompleteSoundFile = getString(find("txtSoundFile"), "text");

        Global.flam3Path = getString(find("txtRenderer"), "text");

        Global.randMinTransforms = Integer.parseInt(getString(find("udMinXForms"), "text"));
        Global.randMaxTransforms = Integer.parseInt(getString(find("udMaxXForms"), "text"));

        Global.mutantMinTransforms = Integer.parseInt(getString(find("udMinMutate"), "text"));
        Global.mutantMaxTransforms = Integer.parseInt(getString(find("udMaxMutate"), "text"));

        Global.symmetryType = getInteger(find("cmbSymType"), "selected");
        Global.symmetryOrder = Integer.parseInt(getString(find("udSymOrder"), "text"));
        Global.symmetryNVars = Integer.parseInt(getString(find("udSymNVars"), "text"));

        Global.batchSize = Integer.parseInt(getString(find("udBatchSize"), "text"));

        Global.randomPrefix = getString(find("txtRandomPrefix"), "text");

        Global.defSampleDensity = Double.parseDouble(getString(find("txtSampleDensity"), "text"));
        Global.defGamma = Double.parseDouble(getString(find("txtGamma"), "text"));
        Global.defBrightness = Double.parseDouble(getString(find("txtBrightness"), "text"));
        Global.defVibrancy = Double.parseDouble(getString(find("txtVibrancy"), "text"));
        Global.defFilterRadius = Double.parseDouble(getString(find("txtFilterRadius"), "text"));

        Global.defOversample = Integer.parseInt(getString(find("txtOversample"), "text"));

        Global.prevLowQuality = Double.parseDouble(getString(find("txtLowQuality"), "text"));
        Global.prevMediumQuality = Double.parseDouble(getString(find("txtMediumQuality"), "text"));
        Global.prevHighQuality = Double.parseDouble(getString(find("txtHighQuality"), "text"));

        Global.editorBkgColor = getColor(find("btnBackgroundColor"), "background").getRGB();
        Global.referenceTriangleColor = getColor(find("btnReferenceColor"), "background").getRGB();
        Global.helpersColor = getColor(find("btnHelperColor"), "background").getRGB();
        Global.gridColor1 = getColor(find("btnGridColor1"), "background").getRGB();
        Global.gridColor2 = getColor(find("btnGridColor2"), "background").getRGB();

        Global.useTransformColors = getBoolean(find("chkUseXFormColor"), "selected");
        ...
        if (getInteger(find("cbNrThreads"), "selected") == 0) {
            ...
            Global.nrThreads = Integer.parseInt(getString(find("cbNrThreads"), "text"));
        }

        Global.doubleClickSetVars = getBoolean(find("doubleclick1"), "selected");

        Global.jpegQuality = Integer.parseInt(getString(find("txtJPEGQuality"), "text"));
        ...
        Global.minNodes = Integer.parseInt(getString(find("udMinNodes"), "text"));
        Global.maxNodes = Integer.parseInt(getString(find("udMaxNodes"), "text"));
        ...
        Global.numTries = Integer.parseInt(getString(find("txtNumTries"), "text"));
        Global.tryLength = Integer.parseInt(getString(find("txtTryLength"), "text"));
    ...
    public void btnBrowseSoundClick() {
        ...
        Global.opendialog = new OpenDialog(this, Global.browserPath, task);
        Global.opendialog.addFilter("Audio interchange files (*.aif,*.aiff)", "*.aif;*.aiff");
    ...
    public void btnBrowseWatermarkClick() {
        ...
        Global.opendialog = new OpenDialog(this, Global.browserPath, task);
        Global.opendialog.addFilter("JPEG files (*.jpg,*.jpeg)", "*.jpg;*.jpeg");
