...
switch(scrollmode)
	{
	case modeRotate: applyRotate(value); break;
	case modeHue: applyHue(value); break;
	case modeSaturation: applySaturation(value); break;
	case modeContrast: applyContrast(value); break;
	case modeBrightness: applyBrightness(value); break;
	case modeBlur: applyBlur(value); break;
	case modeFrequency: applyFrequency(value); break;
	}
...
for(int i=0;i<256;i++)
	{
	hsv = Color.RGBtoHSB(backupal[i][0],backupal[i][1],backupal[i][2],hsv);
	if(hsv[1]!=0)
		{
...
switch(scrollmode)
	{
	case modeRotate: setScroll(-128,128,0); break;
	case modeHue: setScroll(-180,180,0); break;
	case modeSaturation: setScroll(-100,100,0); break;
	case modeContrast: setScroll(-100,100,0); break;
	case modeBrightness: setScroll(-255,255,0); break;
	case modeBlur: setScroll(0,127,0); break;
	case modeFrequency: setScroll(1,10,1); break;
	}
...
if(!previewonly)
	...
	setInteger(find("scrollBrightness"),"value",(int)(cp.brightness*100));
	setString(find("txtBrightness"),"text",""+round6(cp.brightness));

	setInteger(find("scrollVibrancy"),"value",(int)(cp.vibrancy*100));
	...
	setInteger(find("scrollAngle"),"value",(int)(cp.fangle*18000/Math.PI));
	setString(find("txtAngle"),"text",""+(cp.fangle*180/Math.PI));

	if((Math.abs(cp.center[0])<1000)&&(Math.abs(cp.center[1])<1000))
		{
		setInteger(find("scrollCenterX"),"value",(int)(cp.center[0]*1000));
		setString(find("txtCenterX"),"text",""+round6(cp.center[0]));

		setInteger(find("scrollCenterY"),"value",(int)(cp.center[1]*1000));
	...
	setColor(find("btnBackground"),"background",
		new Color(cp.background[0],cp.background[1],cp.background[2]));
	...
	if(cp.cmapindex>=0)
		...
		setString(find("cmbPalette"),"text",CMap.cmapnames[cp.cmapindex]);
		}
	else
		{
	...
	setBoolean(find("chkMaintainRatio"),"selected",Global.maintainRatio);
...
try	{
	cp.zoom =
		Double.valueOf(getString(find("txtZoom"),"text")).doubleValue();
	cp.center[0] =
		Double.valueOf(getString(find("txtCenterX"),"text")).doubleValue();
	cp.center[1] =
		Double.valueOf(getString(find("txtCenterY"),"text")).doubleValue();
	cp.fangle =
		Double.valueOf(getString(find("txtAngle"),"text")).doubleValue()*Math.PI/180.0;
...
Color bg = new Color(cp.background[0],cp.background[1],cp.background[2]);
...
saveGradient(Global.entrydialog.filename,Global.entrydialog.entryname);
}
