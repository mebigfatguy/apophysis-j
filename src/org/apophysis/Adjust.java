...
public class Adjust extends MyThinlet implements Constants, ThreadTarget {
    ...
    Adjust(String title, String xmlfile, int width, int height) throws Exception {
    ...
    public void updateDisplay(boolean previewonly) {
        ...
        if (!previewonly) {
            ...
            setColor(find("btnBackground"), "background", new Color(cp.background[0], cp.background[1], cp.background[2]));
            ...
            for (int i = 0; i < 3; i++) {
                setString(find("btnPreset" + (i + 1)), "text", Global.sizepresets[i][2] + " x " + Global.sizepresets[i][3]);
            }
