...
if(sample_density==0) sample_density = 0.001;
...
lsa[0] = 0;
for(i=1;i<=1024;i++)
	lsa[i] = (k1*Math.log(1+fcp.white_level*i*k2)/Math.log(10))/(fcp.white_level*i);
...
for(i=0;i<fcp.height;i++)
	{
	bx = 0;
	if((i%7)==0) progress(i*1.0/fcp.height);
	...
	for(j=0;j<fcp.width;j++)
		...
		if(filtersize>1)
			...
			ls = lsa[Math.min(1023,buckets[ind+iCount])] / PREFILTER_WHITE;
		...
		if(fcp.transparency)
			...
			if(notvib>0)
				{
				ri = (int)(ls*fp[0]+notvib*Math.pow(fp[0],gamma));
				gi = (int)(ls*fp[1]+notvib*Math.pow(fp[1],gamma));
				bi = (int)(ls*fp[2]+notvib*Math.pow(fp[2],gamma));
				}
			else
				{
				ri = (int)(ls*fp[0]);
			...
			pixels[irow+j] = rbits[ri]|gbits[gi]|bbits[bi]|abits[ai];
			...
			if(notvib>0)
				{
				ri = (int)(ls*fp[0]+notvib*Math.pow(fp[0],gamma)+0.5);
				gi = (int)(ls*fp[1]+notvib*Math.pow(fp[1],gamma)+0.5);
				bi = (int)(ls*fp[2]+notvib*Math.pow(fp[2],gamma)+0.5);
				}
			else
				{
				ri = (int)(ls*fp[0]+0.5);
...
public void setBucketData(int buckets[], int bucketwidth,
	int bucketheight, int bits)
{
...
Color bg = new Color(fcp.background[0],fcp.background[1],fcp.background[2]);
...
if(format.equals("jpg")&&(comment))
	...
	JpegCommentOutputStream os = new JpegCommentOutputStream(
		new File(filename), scomment);
	...
	{
	String scomment = buildComment(encrypt);
	PngCommentOutputStream os = new PngCommentOutputStream(
		new File(filename),scomment);
	ImageIO.write(bimage,format,os);
	os.close();
	}
...
{
if(Global.watermarkFile.length()==0) return;
