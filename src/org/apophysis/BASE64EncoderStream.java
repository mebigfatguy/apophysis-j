...
/**
 * This class implements a BASE64 encoder. It is implemented as a FilterOutputStream, so one can just wrap this class around any output stream and write bytes
 * into this filter. The encoding is done as the bytes are written out.
 *
 * @author John Mani
 * @author Bill Shannon
 */
...
public class BASE64EncoderStream extends FilterOutputStream {
     ...
     *            the output stream
     * @param bytesPerLine
     *            number of bytes per line. The encoder inserts a CRLF sequence after the specified number of bytes, unless bytesPerLine is Integer.MAX_VALUE,
     *            in which case no CRLF is inserted. bytesPerLine is rounded down to a multiple of 4.
     */
    ...
    /**
     * Base64 encode a byte array. No line breaks are inserted. This method is suitable for short strings, such as those in the IMAP AUTHENTICATE protocol, but
     * not to encode the entire content of a MIME part.
     */
    ...
    /**
     * Internal use only version of encode. Allow specifying which part of the input buffer to encode. If outbuf is non-null, it's used as is. Otherwise, a new
