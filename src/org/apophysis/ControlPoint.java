...
static int sym_distrib[] = {-4,-3,-2,-2,-2,-1,-1,-1,2,2,2,3,3,4,4};
...
spatial_filter_radius = getHashDouble(h,"p_spat_filt_rad",spatial_filter_radius);
...
for(int i=0;i<nxforms;i++)
	...
	xform[i].symmetry = getHashDouble(h,prefix+"sym",xform[i].symmetry);
	...
	for(int j=0;j<nv;j++)
		...
		for(int k=0;k<np;k++)
			{
			key = prefix + "par_"+ XForm.getVariation(j).getParameterName(k);
...
if(data.length()==count*6)
	...
	for(int i=0;i<count;i++)
		{
		cmap[i][0] = Integer.parseInt(data.substring(i*6,i*6+2),16);
		cmap[i][1] = Integer.parseInt(data.substring(i*6+2,i*6+4),16);
		cmap[i][2] = Integer.parseInt(data.substring(i*6+4,i*6+6),16);
		}
	}
	...
	for(int i=0;i<count;i++)
		{
		cmap[i][0] = Integer.parseInt(data.substring(i*8+2,i*8+4),16);
		cmap[i][1] = Integer.parseInt(data.substring(i*8+4,i*8+6),16);
		cmap[i][2] = Integer.parseInt(data.substring(i*8+6,i*8+8),16);
		}
	}
...
if(Global.referenceMode>0)
	...
	for(i=0;i<n;i++)
		...
		for(j=0;j<=5;j++)
			{
			temp_x = xset*xform[i].c00 + yset*xform[i].c10 + xform[i].c20;
			temp_y = xset*xform[i].c01 + yset*xform[i].c11 + xform[i].c21;
			xset = temp_x;
			yset = temp_y;
			}
		if(i==0)
...
for(int i=0;i<=t;i++)
	...
	if(xform[i].postXswap)
		{
		solve3(triangles[M1].x[0], -triangles[M1].y[0], triangles[i].x[0],
			triangles[M1].x[1], -triangles[M1].y[1], triangles[i].x[1],
			triangles[M1].x[2], -triangles[M1].y[2], triangles[i].x[2],
			abe);
		xform[i].p00 = abe[0];
		xform[i].p10 = abe[1];
		xform[i].p20 = abe[2];

		solve3(triangles[M1].x[0], -triangles[M1].y[0], -triangles[i].y[0],
			triangles[M1].x[1], -triangles[M1].y[1], -triangles[i].y[1],
			triangles[M1].x[2], -triangles[M1].y[2], -triangles[i].y[2],
			abe);
		xform[i].p01 = abe[0];
		xform[i].p11 = abe[1];
		xform[i].p21 = abe[2];
...
double solve3(double x1, double x2, double x1h,
	double y1, double y2, double y1h,
	double z1, double z2, double z1h,
	double abe[])
{
double det1;
...
if(det1!=0.0)
	{
    abe[0] = (x1h * det(y2, 1.0, z2, 1.0) - x2 * det(y1h, 1.0, z1h, 1.0)
      + 1 * det(y1h, y2, z1h, z2)) / det1;
    abe[1] = (x1 * det(y1h, 1.0, z1h, 1.0) - x1h * det(y1, 1.0, z1, 1.0)
      + 1 * det(y1, y1h, z1, z1h)) / det1;
    abe[2] = (x1 * det(y2, y1h, z2, z1h) - x2 * det(y1, y1h, z1, z1h)
...
for(int i=0;i<=2;i++)
	...
	t.x[i] = Global.mainTriangles[M1].x[i]*xform[n].c00
		-Global.mainTriangles[M1].y[i]*xform[n].c10 + xform[n].c20;

	t.y[i] = Global.mainTriangles[M1].x[i]*xform[n].c01
		+Global.mainTriangles[M1].y[i]*xform[n].c11 - xform[n].c21;
	}
}
...
for(int i=0;i<=2;i++)
	...
	t.x[i] = Global.mainTriangles[M1].x[i]*xform[n].p00
		-Global.mainTriangles[M1].y[i]*xform[n].p10 + xform[n].p20;

	t.y[i] = Global.mainTriangles[M1].x[i]*xform[n].p01
		+Global.mainTriangles[M1].y[i]*xform[n].p11 - xform[n].p21;
	}
}
...
for(int i=0;i<256;i+=8)
	...
	for(int j=0;j<8;j++) {
		for(int k=0;k<3;k++)
			{
			String s = Integer.toHexString(cmap[i+j][k]).toUpperCase();
			if(s.length()<2) {
				s = "0"+s;
			}
			w.print(s);
...
for(int i=0;i<10;i++) {
	System.out.println("cmap"+i+" "+cmap[i][0]+" "+cmap[i][1]+" "+cmap[i][2]);
}
System.out.println("-----------------------------------------------------");
}
...
while(true)
	...
	if((rnd>=0)&&(rnd<=6))
		...
		for(int i=0;i<transforms;i++)
			...
			cp.xform[i].translate(Global.random()*2-1,Global.random()*2-1);
			...
			if(ii==0) {
				cp.xform[i].multiply(1,Global.random()-0.5,Global.random()-0.5,1);
			}
			cp.setVariation(Global.variation);
			}
		}
		...
		for(int i=0;i<transforms;i++)
			...
			cp.xform[i].c10 = s*(Math.cos(theta)*Math.cos(phi)-Math.sin(theta));
			cp.xform[i].c01 = r*Math.sin(theta);
			cp.xform[i].c11 = s*(Math.sin(theta)*Math.cos(phi)+Math.cos(theta));
...
if((maxx-minx>0.001)&&(maxy-miny>0.001)) {
	pixels_per_unit = 0.65*Math.min(width*1./(maxx-minx),height*1./(maxy-miny));
} else {
...
if(Global.preserveQuality)
	...
	pixels_per_unit = pixels_per_unit*width/Math.abs(r.right-r.left);
}
...
if(Global.preserveQuality)
	{
	double z = Math.pow(2,zoom)/(width/(Math.abs(r.right-r.left)+1));
...
cpr.spatial_filter_radius = c0*cp1.spatial_filter_radius+
	c1*cp2.spatial_filter_radius;

cpr.center[0] = c0*cp1.center[0] + c1*cp2.center[0];
cpr.center[1] = c0*cp1.center[1] + c1*cp2.center[1];

cpr.pixels_per_unit = c0*cp1.pixels_per_unit + c1*cp2.pixels_per_unit;
cpr.zoom = c0*cp1.zoom + c1*cp2.zoom;

cpr.nbatches = (int)Math.round(c0*cp1.nbatches+c1*cp2.nbatches);
cpr.white_level = (int)Math.round(c0*cp1.white_level+c1*cp2.white_level);

for(int i=0;i<=3;i++)
	{
	cpr.pulse[i/2][i%2] = c0*cp1.pulse[i/2][i%2] + c1*cp2.pulse[i/2][i%2];
	cpr.wiggle[i/2][i%2] = c0*cp1.wiggle[i/2][i%2] + c1*cp2.wiggle[i/2][i%2];
	}
...
for(int i=0;i<=NXFORMS;i++)
	{
	cpr.xform[i].density = c0*cp1.xform[i].density + c1*cp2.xform[i].density;
	cpr.xform[i].color = c0*cp1.xform[i].color + c1*cp2.xform[i].density;
	for(int j=0;j<nv;j++) {
		cpr.xform[i].vars[j] = c0*cp1.xform[i].vars[j]+c1*cp2.xform[i].vars[j];
	}
	for(int j=0;j<np;j++) {
		cpr.xform[i].pvalues[j] =
			c0*cp1.xform[i].pvalues[j] + c1*cp2.xform[i].pvalues[j];
	}
