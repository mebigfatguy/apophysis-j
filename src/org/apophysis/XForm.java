...
if(coefs!=null) if(coefs.length==6)
	{
	c00 = coefs[0];
	c01 = coefs[1];
	c10 = coefs[2];
...
if(needAngle) v.addElement(new AngleComputation());
if(needLength) v.addElement(new LengthComputation());
if(needSine) v.addElement(new SineComputation());
...
for(int i=1;i<nv;i++)
	if(vars[i]!=0) return true;

if(vars[0]!=1) return true;
...
/*
System.out.println("AFTER APPLY c= "+c00+" "+c10+" "+c20+" "+c01+" "+c11+" "+c21);
*/
...
for(int i=0;i<nv;i++)
	if(vars[i]==0)
		...
		for(int k=0;k<kp;k++)
			pvalues[np++] =
				variations[i].getParameterValue(k);
		}
