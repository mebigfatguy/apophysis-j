...
public class Editor extends MyThinlet implements Constants, ThreadTarget {
    ...
    static final int[] trgColors = { 0xFF0000, 0xFFFF00, 0x00FF00, 0x00FFFF, 0x4040FF, 0xFF00FF, 0xFF7F00, 0xFF007F, 0xFFFF55, 0xCCFFCC, 0xAAFFFF, 0xFF7F7F,
            0xFFAAFF, 0xFFCC55 };
    ...
    Editor(String title, String xmlfile, int width, int height) throws Exception {
    ...
    public void triangleViewMouseDrag(MouseEvent e, Rectangle bounds) {
        ...
        switch (action) {
            ...
            case ACTION_SCALE:
                ...
                dy = ((cclick * sclick * (fdrag[0] - rcenter.x)) + (sclick * sclick * fdrag[1]) + (cclick * cclick * rcenter.y)) / det;
                dy = dy - rcenter.y;
                dx = ((cclick * sclick * (fdrag[1] - rcenter.y)) + (cclick * cclick * fdrag[0]) + (sclick * sclick * rcenter.x)) / det;
                dx = dx - rcenter.x;
                t.copy(oldT);
                if (cclick != 0) {
            ...
            case ACTION_SLIDE:
                ...
                dy = ((cclick * sclick * (fdrag[0] - pivot.x)) + (sclick * sclick * fdrag[1]) + (cclick * cclick * pivot.y)) / det;
                dx = ((cclick * sclick * (fdrag[1] - pivot.y)) + (cclick * cclick * fdrag[0]) + (sclick * sclick * pivot.x)) / det;
                t.copy(oldT);
    ...
    void updateParameterList() {
        ...
        for (int i = 0; i < nv; i++) {
            ...
            for (int j = 0; j < np; j++) {
                ...
                setColor(find("lblParameter" + kp), "foreground", ok ? Color.black : Color.gray);
    ...
    public void cornerEditExit() {
        ...
        try {
            Global.mainTriangles[selectedTriangle].x[0] = Double.parseDouble(getString(find("txtAx"), "text"));

            Global.mainTriangles[selectedTriangle].y[0] = Double.parseDouble(getString(find("txtAy"), "text"));

            Global.mainTriangles[selectedTriangle].x[1] = Double.parseDouble(getString(find("txtBx"), "text"));

            Global.mainTriangles[selectedTriangle].y[1] = Double.parseDouble(getString(find("txtBy"), "text"));

            Global.mainTriangles[selectedTriangle].x[2] = Double.parseDouble(getString(find("txtCx"), "text"));

            Global.mainTriangles[selectedTriangle].y[2] = Double.parseDouble(getString(find("txtCy"), "text"));
    ...
    public void btTrgScaleClick(int option) {
        ...
        try {
            ...
            getPivot();
            Global.main.updateUndo();
            Global.mainTriangles[selectedTriangle].scaleAroundPoint(pivot.x, pivot.y, scale);
    ...
    Color valueToColor(double value) {
        ...
        return new Color(Global.mainCP.cmap[ic][0], Global.mainCP.cmap[ic][1], Global.mainCP.cmap[ic][2]);
    }
    ...
    public void scrollSymmetryChange(Object slider) {
        ...
        if (cp.xform[selectedTriangle].symmetry != value) {
            cp.xform[selectedTriangle].symmetry = value;
            setString(find("txtSymmetry"), "text", "" + cp.xform[selectedTriangle].symmetry);
        }
    ...
    Image buildTriangleIcon(int color) {
        ...
        int[] pixels = new int[] { rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0,
                rgb0, rgb1, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb1, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1,
                rgb2, rgb1, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb1, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2,
                rgb2, rgb1, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb2, rgb2, rgb1, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb2, rgb2,
                rgb2, rgb1, rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb2, rgb2, rgb2, rgb2, rgb1, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb2, rgb2, rgb2, rgb2,
                rgb2, rgb1, rgb0, rgb0, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0,
                rgb0, rgb0, rgb0 };
    ...
    int getWidget(double x, double y) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 2; j++) {
                double d = distanceToSegment(fmove[0], fmove[1], widgets[i][j][0], widgets[i][j][1], widgets[i][j + 1][0], widgets[i][j + 1][1]);
    ...
    static double distanceToSegment(double x, double y, double x1, double y1, double x2, double y2) {
        double a, b, e, c;
