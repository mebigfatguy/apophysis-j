...
setBoolean(find("tbEnableFinalXform"),"selected",Global.enableFinalXform);
...
switch(action)
	...
	case ACTION_SCALE:
		for(int i=0;i<3;i++)
			{
			ix1 = XScreen(rcenter.x+(t.x[i]-rcenter.x)*sc,ix,gCenterX,sc);
			iy1 = YScreen(rcenter.y+(t.y[i]-rcenter.y)*sc,iy,gCenterY,sc);
			ix2 = XScreen(rcenter.x-(t.x[i]-rcenter.x)*sc,ix,gCenterX,sc);
			iy2 = YScreen(rcenter.y-(t.y[i]-rcenter.y)*sc,iy,gCenterY,sc);
			g.drawLine(ix1,iy1,ix2,iy2);
			}
		break;
...
if(vimage==null)
	...
	source = new MemoryImageSource(graphwidth,graphheight,pixels,0,graphwidth);
...
for(int ax=-n;ax<=n;ax++) {
	for(int ay=-n;ay<=n;ay++)
		...
		if((a_x>=0)&&(a_x<graphwidth)&&(a_y>=0)&&(a_y<graphheight)) {
...
bgcolor = new Color(cp.background[0],cp.background[1],cp.background[2]);
...
/*
for(int i=0;i<cp.nxforms+1;i++)
	cp.xform[i].print("form"+i+" = ");
*/
...
if(it==Global.transforms)
	...
	Global.mainTriangles[Global.transforms].copy(Global.mainTriangles[M1]);
	...
	if(it==Global.transforms-1)
		{
		Global.mainTriangles[it].copy(Global.mainTriangles[Global.transforms]);
...
switch(action)
	...
	case ACTION_SCALE:
		...
		dy = (cclick*sclick*(fdrag[0]-rcenter.x) +
			sclick*sclick*fdrag[1] + cclick*cclick*rcenter.y)/det;
		dy = dy-rcenter.y;
		dx = (cclick*sclick*(fdrag[1]-rcenter.y) +
			cclick*cclick*fdrag[0] + sclick*sclick*rcenter.x)/det;
		dx = dx-rcenter.x;
		t.copy(oldT);
		if(cclick!=0)
			{
	...
	case ACTION_SCALE_EDGE:
		...
		t.copy(oldT);
		t.scaleCornerAroundPoint(selectedCorner,pivot.x,pivot.y,ddrag/dclick);
	...
	case ACTION_ROTATE_EDGE:
		...
		t.copy(oldT);
		t.x[selectedCorner] = rcenter.x + (fdrag[0]-rcenter.x)*rradius/ddrag;
		t.y[selectedCorner] = rcenter.y + (fdrag[1]-rcenter.y)*rradius/ddrag;
	...
	case ACTION_SLIDE:
		...
		dy = (cclick*sclick*(fdrag[0]-pivot.x) +
			sclick*sclick*fdrag[1] + cclick*cclick*pivot.y)/det;
		dx = (cclick*sclick*(fdrag[1]-pivot.y) +
			cclick*cclick*fdrag[0] + sclick*sclick*pivot.x)/det;
		t.copy(oldT);
...
/*
void unscale(int xy[], double fx, double fy, Rectangle bounds)
{
double sc = 50*graphZoom;
xy[0] = bounds.width/2 + (fx-gCenterX)*sc ;
xy[1] = bounds.height/2 - (fy+gCenterY)*sc ;
}
*/
...
for(int i=0;i<nv;i++)
	...
	*/
	...
	setMethod(field,"perform","updateVariations()", getDesktop(),this);

	Object button = createImpl("button");
	setString(button,"text",XForm.getVariation(i).getName());
	putProperty(button,"field",field);
	setMethod(button,"action","toggleVariation(this)",getDesktop(),this);
	/*
	add(list,label);
	*/
	add(list,button);
...
if(s.length()==0) {
	...
	{
	double value = 0.0;
	try { value = Double.valueOf(s).doubleValue(); } catch(Exception ex) {}
	if(value==0.0) {
		setString(field,"text","1");
	} else {
...
for(int i=0;i<nv;i++)
	...
	for(int j=0;j<np;j++)
		...
		setMethod(field,"perform","updateParameters()", getDesktop(),this);
...
if(pivotMode==PIVOT_LOCAL)
	...
	pivot.x = t.x[1]+(t.x[0]-t.x[1])*localPivot.x+(t.x[2]-t.x[1])*localPivot.y;
	pivot.y = t.y[1]+(t.y[0]-t.y[1])*localPivot.x+(t.y[2]-t.y[1])*localPivot.y;
	}
else
	{
...
try	{
	...
	Global.mainTriangles[selectedTriangle].move(dx*offset,dy*offset);
...
setInteger(find("scrollSymmetry"),"value",(int)(xform.symmetry*1000));
...
if(getBoolean(find("btnCoefsRect"),"selected"))
	...
	setString(find("txtB"),"text",format(arctan2(-xform.c01,xform.c00)));
	setString(find("txtC"),"text",format(hypot(xform.c10,xform.c11)));
	setString(find("txtD"),"text",format(arctan2(xform.c11,-xform.c10)));
	setString(find("txtE"),"text",format(hypot(xform.c20,xform.c21)));
	setString(find("txtF"),"text",format(arctan2(-xform.c21,xform.c20)));

	setString(find("txtPost00"),"text",format(hypot(xform.p00,xform.p01)));
	setString(find("txtPost01"),"text",format(arctan2(-xform.p01,xform.p00)));
	setString(find("txtPost10"),"text",format(hypot(xform.p10,xform.p11)));
	setString(find("txtPost11"),"text",format(arctan2(xform.p11,-xform.p10)));
	setString(find("txtPost20"),"text",format(hypot(xform.p20,xform.p21)));
	setString(find("txtPost21"),"text",format(arctan2(-xform.p21,xform.p20)));
	}
...
setString(find("trkVarPreviewDensity"),"tooltip","Density: "+varPreviewDensity);

setInteger(find("trkVarPreviewRange"),"value",varPreviewRange);
setString(find("trkVarPreviewRange"),"tooltip","Range: "+varPreviewRange);

setInteger(find("trkVarPreviewDepth"),"value",varPreviewDepth);
setString(find("trkVarPreviewDepth"),"tooltip","Depth: "+varPreviewDepth);
...
if((index!=selectedTriangle)&(index>=0)&(index<=lastTriangle()))
	{
...
try	{
	Global.mainTriangles[selectedTriangle].x[0] =
		Double.valueOf(getString(find("txtAx"),"text")).doubleValue();

	Global.mainTriangles[selectedTriangle].y[0] =
		Double.valueOf(getString(find("txtAy"),"text")).doubleValue();

	Global.mainTriangles[selectedTriangle].x[1] =
		Double.valueOf(getString(find("txtBx"),"text")).doubleValue();

	Global.mainTriangles[selectedTriangle].y[1] =
		Double.valueOf(getString(find("txtBy"),"text")).doubleValue();

	Global.mainTriangles[selectedTriangle].x[2] =
		Double.valueOf(getString(find("txtCx"),"text")).doubleValue();

	Global.mainTriangles[selectedTriangle].y[2] =
		Double.valueOf(getString(find("txtCy"),"text")).doubleValue();
...
if(getBoolean(find("btnCoefsRect"),"selected"))
	...
	try	{ cp.xform[selectedTriangle].c00 = Double.valueOf(s00).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].c01 = -Double.valueOf(s01).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].c10 = -Double.valueOf(s10).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].c11 = Double.valueOf(s11).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].c20 = Double.valueOf(s20).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].c21 = -Double.valueOf(s21).doubleValue(); }
	catch(Exception ex) {}
...
if(getBoolean(find("btnCoefsRect"),"selected"))
	...
	try	{ cp.xform[selectedTriangle].p00 = Double.valueOf(s00).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].p01 = -Double.valueOf(s01).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].p10 = -Double.valueOf(s10).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].p11 = Double.valueOf(s11).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].p20 = Double.valueOf(s20).doubleValue(); }
	catch(Exception ex) {}

	try	{ cp.xform[selectedTriangle].p21 = -Double.valueOf(s21).doubleValue(); }
	catch(Exception ex) {}
...
try	{
	...
	getPivot();
	Global.main.updateUndo();
	Global.mainTriangles[selectedTriangle].scaleAroundPoint(
			pivot.x, pivot.y,scale);
...
try	{
	value = Double.valueOf(getString(find("editPivotX"),"text")).doubleValue();
	if(pivotMode==PIVOT_LOCAL) {
		localPivot.x = value;
	} else {
		worldPivot.x = value;
...
try	{
	value = Double.valueOf(getString(find("editPivotY"),"text")).doubleValue();
	if(pivotMode==PIVOT_LOCAL) {
		localPivot.y = value;
	} else {
		worldPivot.y = value;
...
Global.mainTriangles[it].x[0] = 2*pivot.x - Global.mainTriangles[it].x[0];
Global.mainTriangles[it].x[1] = 2*pivot.x - Global.mainTriangles[it].x[1];
Global.mainTriangles[it].x[2] = 2*pivot.x - Global.mainTriangles[it].x[2];
...
Global.mainTriangles[it].y[0] = 2*pivot.y - Global.mainTriangles[it].y[0];
Global.mainTriangles[it].y[1] = 2*pivot.y - Global.mainTriangles[it].y[1];
Global.mainTriangles[it].y[2] = 2*pivot.y - Global.mainTriangles[it].y[2];
...
{
try	{
	double value = Double.valueOf(getString(field,"text")).doubleValue();
...
if(cp.xform[selectedTriangle].color!=value)
	...
	cp.xform[selectedTriangle].color = value;
	setColor(find("pnlColor"),"background",valueToColor(value));
	setString(find("txtColor"),"text",""+cp.xform[selectedTriangle].color);
	}
...
return new Color(
	Global.mainCP.cmap[ic][0],
	Global.mainCP.cmap[ic][1],
...
try	{
	...
	if(cp.xform[selectedTriangle].symmetry!=value)
		...
		setInteger(find("scrollSymmetry"),"value",(int)(1000*value));
...
int pixels[] = new int[]{
    rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0,
    rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb0,
    rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb1, rgb0,
    rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb1, rgb0,
    rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb1, rgb0,
    rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb2, rgb1, rgb0,
    rgb0, rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb2, rgb2, rgb1, rgb0,
    rgb0, rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb2, rgb2, rgb2, rgb1, rgb0,
    rgb0, rgb0, rgb0, rgb1, rgb2, rgb2, rgb2, rgb2, rgb2, rgb2, rgb1, rgb0,
    rgb0, rgb0, rgb1, rgb2, rgb2, rgb2, rgb2, rgb2, rgb2, rgb2, rgb1, rgb0,
    rgb0, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb1, rgb0,
    rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0, rgb0
    };
...
for(k=lastTriangle();k>=0;k--)
	...
	for(int i=0;i<=2;i++)
		...
		if((((t.y[i]<=y)&&
			...
			((t.y[j]<=y)&&
			(y<t.y[i])))&&
			(x<(t.x[j]-t.x[i])*(y-t.y[i])/(t.y[j]-t.y[i])+t.x[i])) {
			inside = !inside;
		}
		j = i;
		}
...
switch(option)
	{
	case 0 : varPreviewRange = value; break;
	case 1 : varPreviewDensity = value; break;
	case 2 : varPreviewDepth = value; break;
	}
...
{
if((cp.xform[selectedTriangle].c00==1)&&(cp.xform[selectedTriangle].c01==0)) {
	return;
}
...
{
if((cp.xform[selectedTriangle].c10==0)&&(cp.xform[selectedTriangle].c11==1)) {
	return;
}
...
if((t.x[0]==nx0)&&(t.y[0]==ny0)&&(t.x[2]==nx2)&&(t.y[2]==ny2)) {
	return;
}
...
{
try	{
	double newval = Double.valueOf(getString(field,"text")).doubleValue();
