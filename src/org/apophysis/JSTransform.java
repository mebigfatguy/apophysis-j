...
for(int i=0;i<nv;i++)
	...
	for(int j=0;j<np;j++)
		{
		ScriptableObject.putProperty(this,
			XForm.getVariation(i).getParameterName(j),
			Context.javaToJS(new Double(Variable[kp]),this));
		kp++;
		}
	}
...
for(int i=0;i<nv;i++)
	{
	o = ScriptableObject.getProperty(this,XForm.getVariation(i).getName());
...
for(int i=0;i<nv;i++)
	...
	for(int j=0;j<np;j++)
		{
		o = ScriptableObject.getProperty(this,
			XForm.getVariation(i).getParameterName(j));
		double value1 = getValue(o);
