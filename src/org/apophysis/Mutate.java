...
public class Mutate extends MyThinlet implements Constants, ThreadTarget {
    ...
    Mutate(String title, String xmlfile, int width, int height) throws Exception {
    ...
    public void drawImage(Graphics g, Rectangle bounds, int index) {
        ...
        Color bg = new Color(cps[0].background[0], cps[0].background[1], cps[0].background[2]);
        g.setColor(bg);
    ...
    public void mutantClick(MouseEvent e, int index, Object canvas, Rectangle bounds) {
        if (e.isPopupTrigger()) {
            Rectangle r = new Rectangle(bounds);
            setToAbsolutePosition(canvas, r);
            Object popup = find("PreviewPopup");
