...
/**
 * This class implements a BASE64 Decoder. It is implemented as a FilterInputStream, so one can just wrap this class around any input stream and read bytes from
...
public class BASE64DecoderStream extends FilterInputStream {
    ...
    /**
     * Create a BASE64 decoder that decodes the specified input stream. The System property <code>mail.mime.base64.ignoreerrors</code> controls whether errors
     * in the encoded data cause an exception or are ignored. The default is false (errors cause exception).
     *
     * @param in
     *            the input stream
     */
    ...
    /**
     * Read the next decoded byte from this input stream. The byte is returned as an <code>int</code> in the range <code>0</code> to <code>255</code>. If no
     * byte is available because the end of the stream has been reached, the value <code>-1</code> is returned. This method blocks until input data is
    ...
    /**
     * Reads up to <code>len</code> decoded bytes of data from this input stream into an array of bytes. This method blocks until some input is available.
     * <p>
     *
     * @param buf
    ...
    /**
     * Returns the number of bytes that can be read from this input stream without blocking. However, this figure is only a close approximation in case the
     * original encoded stream contains embedded CRLFs; since the CRLFs are discarded, not decoded
     */
    @Override
    ...
    /**
     * The decoder algorithm. Most of the complexity here is dealing with error cases. Returns the number of bytes decoded, which may be zero. Decoding is done
     * by filling an int with 4 6-bit values by shifting them in from the bottom and then extracting 3 8-bit bytes from the int by shifting them out from the
     * bottom.
     *
     * @param outbuf
     ...
     * @return the number of bytes filled, always a multiple of three, and may be zero
     * @exception IOException
                    ...
                    if (i == -1) {
                        ...
                        if (!ignoreErrors) {
                            throw new IOException(
                                    "Error in encoded stream: " + "needed 4 valid base64 characters " + "but only got " + got + " before EOF" + recentChars());
                        }
                        ...
                        if ((got < 2) && !ignoreErrors) {
                            throw new IOException("Error in encoded stream: " + "needed at least 2 valid base64 characters," + " but only got " + got
                    ...
                    while (got < 4) {
                        if (!atEOF) {
                            ...
                            if (i == -1) {
                                if (!ignoreErrors) {
                                    throw new IOException(
                                            "Error in encoded stream: " + "hit EOF while looking for " + "padding characters (=)" + recentChars());
                                }
                            } else if (i != -2) {
                                if (!ignoreErrors) {
                                    throw new IOException(
                                            "Error in encoded stream: " + "found valid base64 character after " + "a padding character (=)" + recentChars());
                                }
                            }
                        }
                        val <<= 6;
    ...
    /**
     * Base64 decode a byte array. No line breaks are allowed. This method is suitable for short strings, such as those in the IMAP AUTHENTICATE protocol, but
     * not to decode the entire content of a MIME part.
     *
