...
if(false)
	...
	System.out.println("fcp.center = "+fcp.center[0]+" "+fcp.center[1]);
	System.out.println("fcp.width="+fcp.width+"  fcp.height="+fcp.height);
	System.out.println("ppux = "+ppux+" ppuy="+ppuy);
	System.out.println("oversample="+oversample);
	System.out.println("t0="+t0+" t1="+t1+" t2="+t2+" t3="+t3);
	System.out.println("camX0="+camX0+"  camX1="+camX1+"  camW="+camW);
	System.out.println("camY0="+camY0+"  camY1="+camY1+"  camH="+camH);
...
if(false)
	...
	System.out.println("fcp.spatial_oversample="+fcp.spatial_oversample);
...
try	{
	...
	if(buckets==null)
		...
		if(bucketsize*4*4>usable)
			{
			bucketslice = (int)(usable/bucketwidth/4/4);
			bucketslice = bucketslice/oversample*oversample;
			bucketsize = bucketslice*bucketwidth;
...
if(fstop==0)
	{
	target.output("  Creating image with quality = "+fcp.actual_density+"\n");
	imager.createImage();
	}
...
for(int i=0;i<=nb;i++)
	...
	if(false)	
	if(firstp) System.out.println("p = "+p[0]+" "+p[1]+" "+p[2]+" ix="+ix+" iy="+iy);
	firstp = false;
	nu++;
	}
...
nsamples = (int)Math.round(sample_density*nrslices*bucketsize/(oversample*oversample));


fnumbatches = (int)Math.round(nsamples/(fcp.nbatches*SUB_BATCH_SIZE));
if(fnumbatches==0)
	fnumbatches = 1;


/*
fminbatches = (int)Math.round(fnumbatches*fmindensity/fcp.sample_density);
if(fminbatches==0)
	fminbatches = 1;
*/
...
if(bucketslice<bucketheight)
	{
	try	{
		out = new FileOutputStream(File.createTempFile("render",".swap"));
		}
...
while(bucketfirst<bucketheight)
	...
	for(int i=0;i<fnumbatches;i++)
		...
		switch(iproc)
			{
			case 0:	iterateBatchFX(incr);		break;
			case 1: iterateBatch(incr);			break;
			case 2:	iterateBatchAngleFX(incr);	break;
			case 3: iterateBatchAngle(incr);	break;
			}	
		}
...
return	
	...
	((min<10)?("0"+min):(""+min)) + ":" +
	((sec<10)?("0"+sec):(""+sec)) + ":" +
	((mil<10)?("00"+mil):((mil<100)?("0"+mil):(""+mil)));
}
