...
public class Renderer implements Constants {
    ...
    void iterateBatch(int incr) {
        ...
        for (int i = 0; i <= nb; i++) {
            ...
            if (false) {
                if (firstp) {
                    System.out.println("p = " + p[0] + " " + p[1] + " " + p[2] + " ix=" + ix + " iy=" + iy);
                }
            }
            firstp = false;
            nu++;
    ...
    void writeBuckets(OutputStream out, int buckets[], int nrow) throws IOException {
    ...
    public void saveImage(String filename, boolean comment, boolean encrypt, boolean watermark) throws IOException {
        try {
